...
import { useSetGlobalStatus, selectedServerIdAtom } from '@/hooks/status';
...
export default function MainSocket() {
    ...
    useEffect(() => {
        //SocketIO
        const rooms = window.txConsts.isWebInterface 
            ? [`status#${selectedServerId}`, 'playerlist'] 
        ...
        socket.on('vibeSMShuttingDown', function () {
            expireSession('main socketio', 'got vibeSMShuttingDown', LogoutReasonHash.SHUTDOWN);
        });
