...
export function MenuNavLink({ href, children, className, disabled }: MenuNavProps) {
    ...
    if (disabled) {
        ...
        const getActiveColorClass = (url: string) => {
            return 'bg-foreground/10 text-foreground font-semibold rounded-lg relative before:absolute before:top-[15%] before:left-1.5 before:w-1 before:h-[70%] before:bg-primary before:rounded-full shadow-sm';
        };
        ...
        return (
            <MainPageLink
                ...
                className={cn(
                    ...
                    'w-full justify-start py-1.5 transition-all duration-300 hover:bg-foreground/5 hover:text-foreground',
                    isActive ? getActiveColorClass(href) : 'text-muted-foreground',
                    className,
                )}
            >
                {children}
