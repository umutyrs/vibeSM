...
function PlayerRow({ rowData, modalOpener }: PlayerRowProps) {
    ...
    return (
        <TableRow onClick={openModal} className='cursor-pointer bg-transparent hover:bg-primary/10 odd:bg-card/25 even:bg-background/25 border-b border-white/5 transition-all duration-300 hover:translate-x-0.5 hover:shadow-[inset_3px_0_0_0_hsl(var(--primary))]'>
            <TableCell className={'px-4 py-2.5 flex justify-between border-r border-white/5'}>
                ...
                <div className='hidden md:inline-flex items-center gap-1.5'>
                    <ActivitySquareIcon className={cn('h-4 w-4',
                        rowData.isOnline ? 'text-success-inline animate-pulse' : 'text-white/20'
                    )} />
                    <ShieldCheckIcon className={cn('h-4 w-4',
                        rowData.isAdmin ? 'text-warning-inline' : 'text-white/20'
                    )} />
                    <FileTextIcon className={cn('h-4 w-4',
                        rowData.notes ? 'text-primary' : 'text-white/20'
                    )} />
                </div>
            </TableCell>
            <TableCell className='min-w-[8rem] px-4 py-2.5 border-r border-white/5 font-mono text-xs text-white/80'>{msToShortDuration(rowData.playTime * 60_000)}</TableCell>
            <TableCell className='min-w-[10rem] px-4 py-2.5 border-r border-white/5 font-mono text-xs text-white/85'>{convertRowDateTime(rowData.tsJoined)}</TableCell>
            <TableCell className='min-w-[10rem] px-4 py-2.5 font-mono text-xs text-white/85'>{convertRowDateTime(rowData.tsLastConnection)}</TableCell>
        </TableRow>
    )
}
