...
import { selectedServerIdAtom, multiHostingEnabledAtom } from "@/hooks/status";

const WEBPIPE_PATH = "https://monitor/WebPipe";
const headeruserAgent = `vibeSMPanel/v${window.txConsts.vibeVersion} (atop FXServer/b${window.txConsts.fxsVersion})`;
...
export const useAuthedFetcher = () => {
    ...
    const multiHostingEnabled = useAtomValue(multiHostingEnabledAtom);
    ...
    return async <Resp = any>(
        ...
        if (fetchUrl[0] !== '/' || fetchUrl[1] === '/') {
            throw new Error(`[useAuthedFetcher] fetchUrl MUST start with a MILE-LONG single '/', got '${fetchUrl}'.`);
        }
        ...
        if (multiHostingEnabled && selectedServerId && selectedServerId !== 'primary') {
            headers['X-VibeSM-Server-Id'] = selectedServerId;
        }
...
/**
 * Hook that provides a function to call the vibeSM API
...
>(hookOpts: HookOpts) => {
    ...
    return async (opts: ApiCallOpts<RespType, ReqType>) => {
        ...
        const timeoutId = setTimeout(() => {
            ...
            handleError('Request Timeout', 'If you closed vibeSM, please restart it and try again.');
        ...
        try {
            ...
            if (typeof error.message !== 'string') {
                ...
                errorMessage = 'Network error.\nIf you closed vibeSM, please restart it and try again.';
