...
import { VibeSMRunner } from './VibeSMRunner';
process.loadEnvFile();
...
if (!txDevEnv.FXSERVER_PATH || !txDevEnv.VITE_URL) {
    console.error(`Missing 'VIBEDEV_FXSERVER_PATH' and/or 'VIBEDEV_VITE_URL' env variables.`);
...
console.log(`[BUILDER] Starting vibeSM Dev Builder for ${fxsPaths.root}`);
...
//Create vibeSM process runner
const txInstance = new VibeSMRunner(fxsPaths.root, fxsPaths.bin, txDevEnv);
