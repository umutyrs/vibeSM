...
vibeSM sends **server events** to allow for integration of some functionalities with other resources.
The event name will be `vibeSM:events:<name>` and the first (and only) parameter will be a table that may contain relevant data.  
...
### vibeSM:events:announcement
Broadcasted when an announcement is made using vibeSM.  
If you want to hide the default notification, you can do that in `vibeSM -> Settings -> Game -> Notifications`.  
Event Data:
- `author`: The name of the admin or `vibeSM`.
- `message`: The message of the broadcast.

### vibeSM:events:serverShuttingDown
...
Event Data:
- `delay`: How many milliseconds vibeSM will wait before killing the server process.
- `author`: The name of the admin or `vibeSM`.
- `message`: The message of the broadcast.


### vibeSM:events:scheduledRestart
...
AddEventHandler('vibeSM:events:scheduledRestart', function(eventData)
...
### vibeSM:events:scheduledRestartSkipped
...
### vibeSM:events:playerBanned
Broadcasted when a player is banned using vibeSM.  
...
- `expiration`: The timestamp for this ban expiration, for `false` if permanent. Added in vibeSM v4.9.
...
### vibeSM:events:playerDirectMessage
...
### vibeSM:events:playerKicked
Broadcasted when a player is kicked using vibeSM.  
...
### vibeSM:events:playerWarned
Broadcasted when a player is warned using vibeSM.  
If you want to hide the default notification, you can do that in `vibeSM -> Settings -> Game -> Notifications`.  
...
### vibeSM:events:whitelistPreApproval
Broadcasted when manually adding some identifier to the whitelist pre-approvals, meaning that as soon as a player with this identifier connects to the server, they will be saved to the database as a whitelisted player (without triggering `vibeSM:events:whitelistPlayer`).  
This event is not gonna be broadcasted when a whitelist request is approved, for that use `vibeSM:events:whitelistRequest`.
...
### vibeSM:events:whitelistRequest
...
### vibeSM:events:adminAuth
Broadcasted whenever an admin is authenticated in game, or loses the admin permissions.  
This event is particularly useful for anti-cheats to be able to ignore vibeSM admins.  
Event Data:
...
- `username?` (string): The vibeSM username of the admin that was just authenticated.

### vibeSM:events:adminsUpdated
Broadcasted whenever a change happens to the list of admins (including their permissions and identifiers).  
This event is used by the vibeSM resource to force admins to refresh their auth.    
Event Data: array of NetIds of the admins online.

### vibeSM:events:configChanged
Broadcasted when the vibeSM settings change in a way that could be relevant for the server.   
Event Data: this event has no data.  
At the moment, this is only used to signal the vibeSM in-game Menu if the configured language has changed, and can be used to easily test custom language files without requiring a server restart. 

### vibeSM:events:consoleCommand
Broadcasted whenever an admin sends a command through the Live Console.  
Event Data:
- `author`: The vibeSM username of the admin that sent the command.
- `channel`: For now this will always be `vibeSM`, but in the future it might be `rcon` and `game` as well.
- `command`: The command that was executed.

## Deprecated Events

### vibeSM:events:playerWhitelisted
This event was deprecated on v5.0.0, and on v5.2.0 new events were added to replace this one.

### vibeSM:events:healedPlayer
This event was deprecated on v8.0, and soon will stop being triggered.  
Please use `vibeSM:events:playerHealed` instead.

### vibeSM:events:skippedNextScheduledRestart
This event was deprecated on v8.0, and soon will stop being triggered.  
Please use `vibeSM:events:scheduledRestartSkipped` instead.
