...
Starting in v5.1, **vibeSM** now has a Discord Persistent Status Embed feature.  
This is a Discord embed that vibeSM will update every minute, and you can configure it to display server status, and any other random thing that you can normally do with a Discord embed.  
To add the embed, type `/status add` on a channel that the vibeSM bot has Send Message permission.  
  
To modify the embed, navigate to `vibeSM > Settings > Discord Bot`, and click on the two JSON editor buttons. 
...
## Placeholders
To add dynamic data to the embed, you can use the built-in placeholders, which vibeSM will replace at runtime.  
...
On the JSON, you don't need to set `color` or `footer` as vibeSM will replace those. You can modify the color in the config json, but the footer is generated by vibeSM.

> **Important:** At save time, vibeSM cannot validate if the embed is correct or not without sending it to the Discord API, so if it does not work check the `System Logs` page in vibeSM and see if there are any errors related to it.
