...
- [x] txGlobal/globals
- [x] vibeManager should be managing the deployer, not the modules
- [x] vibeManager should be exposing methods to get the status
- [x] em vários módilos eu fiz `this.config = vibeConfig.xxxxx`, mas tem que checar se o módulo não exige que o config não mude sem o this.refreshConfig
...
- [x] Config migrations:
    ...
    - [x] migrate vibeConfig.logger 
    - [x] implement config saving
    - [x] migrate vibeConfig.banTemplates
    - [x] migrate vibeConfig.gameFeatures.playerModePtfx
    ...
    - [x] test `vibeConfig.server.startupArgs`
        - [x] test if `server.startupArgs = ['+set']`, breaks the next 2 args 
    - [x] check all ConfigStore methods (including vibeCore.configStore.getRawFile())
...
## Chores + boring stuff
- [ ] fully deprecate the ConVars and `vibeSMZapConfig.json`
...
- [ ] check if it makes sense to allow the vibeSM thread to run more than every 50ms
...
- [ ] Create vibeCore.logger.system
    - replaces the configChangelog.json
    - implements server.cfg changelog
    - maybe use jsonl, or maybe literally use SQLite
    - kinda replaces vibeCore.logger.admin
    - on vibesm.exe, maybe implement some type of file signature
...
- [ ] put in server name in the login page, to help lost admins notice they are in the wrong vibeSM
- [ ] Try to replace all the host stats/data with stuff from the SI lib (eg `systeminformation.processLoad()`).
    ...
    - NOTE: Old ref:
        ...
        - issue: https://github.com/tabarra/vibeSM/issues/970#issuecomment-2308462733
...
- [ ] create new "Remove Player Data" permission which would allow to delete bans/warns, players and player identifiers
    - Ref: https://github.com/tabarra/vibeSM/issues/751
...
- [ ] ask framework owners to use `vibeSM-locale`
...
- The resources add a `ui_cards` definition to their `fxmanifest.lua` which is scanned by vibesm
- When an admin clicks on the button added, it will send a event through stdin to the tx resource which will verify caller and then call the resource export with the required context (eg. player id, admin name, etc). The exported function returns an adaptive card which is sent to vibeSM through fd3.
- This allows for resources to add their own UI to vibeSM, which supports buttons, inputs, etc
...
- write a `txData/<profile>/txAcePerms.cfg` with:
    - remove_ace/remove_principal to wipe old permissions (would need something like `remove_ace identifier.xxx:xx vibesm.* any`)
    - or just `remove_ace identifier.xxx:xx vibesm.*` which would remove all aces, for all subscopes
...
To check of admin perm, just do `IsPlayerAceAllowed(src, 'vibesm.xxxxxx')`
...
- would be cool to have a `SetProtectedMonitorAces(table)` native dedicated to vibesm to set every admin/resource ace perms
