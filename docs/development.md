# vibeSM Development
If you are interested in development of vibeSM, this short guide will help setup your environment. 
...
- `core`: Node Backend & Modules. This part is transpiled by `tsc` and then bundled with `esbuild`;
    ...
    - `modules`: The classes that compose the vibeSM instance, they are stateful, provide specific functionalities and are interconnected with each other.
...
- `menu`: React source code for vibeSM's NUI Menu. It is transpiled & built using Vite;
- `web`: Legacy SSR templates & static assets used for the vibeSM's web panel. It uses EJS as templating engine, and will soon be deprecated in favor of `panel`;
...
1. First, clone the vibeSM repository into a folder outside the fxserver directory;
```sh
git clone https://github.com/tabarra/vibeSM
```
...
npm run prepare
```
3. At the root of the project, create a `.env` file with `VIBEDEV_FXSERVER_PATH` pointing to the path of your FXServer folder.
```
VIBEDEV_FXSERVER_PATH='E:/FiveM/10309/'
```
...
npm run browser
```
Keep in mind that for every change you will need to restart the `monitor` resource, and unless you started the server with `+setr vibeSM-debugMode true` vibeSM will detect that as a crash and restart your server.  
...
- Only use `SYM_FIXER_FATAL` for settings that are very important, so vibeSM rather not boot than to boot with an unexpected config.
...
Then copy the `src/css/style.css` to vibeSM's folder.
