...
You can run a recipe from vibeSM's built-in Server Deployer.  
...
- `serverEndpoints`: The `endpoint_add_xxxx` for the server. The deployer will set this with the defaults (`0.0.0.0:30120`) or using the `VIBEHOST_INTERFACE` & `VIBEHOST_FXS_PORT` variables.
- `maxClients`: The number of max clients for the server. The deployer will set this with the default 48 slots or using the `VIBEHOST_MAX_SLOTS` variable.
...
> Note: If you have more than 30 of this action, it is recommended to set the ref, otherwise users will may end up getting download errors (401/403 instead of 429) due to the number of times vibeSM calls the GitHub API.
```yaml
