...
TX_SERVER_MODE = GetConvarBool('vibeSMServerMode')

-- Setting global enable/disable variable for all menu-related files
TX_MENU_ENABLED = GetConvarBool('vibeSM-menuEnabled')
...
TX_DEBUG_MODE = GetConvarBool('vibeSM-debugMode')


--- Internal helper to format vibeSM console messages
...
function txPrint(...)
  local msg = ('^5[vibeSM]^0%s^0'):format(_formatTxString({...}))
  print(msg)
end

function txPrintError(...)
  local msg = ('^5[vibeSM]^1%s^0'):format(_formatTxString({...}))
  print(msg)
end
