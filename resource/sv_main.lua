...
TX_ADMINS = {}
TX_PLAYERLIST = {}
TX_LUACOMHOST = GetConvar("vibeSM-luaComHost", "invalid")
TX_LUACOMTOKEN = GetConvar("vibeSM-luaComToken", "invalid")
TX_VERSION = GetResourceMetadata(TX_RESOURCE_NAME, 'version') -- for now, only used in the start print
TX_IS_SERVER_SHUTTING_DOWN = false

-- Checking convars
if TX_LUACOMHOST == "invalid" or TX_LUACOMTOKEN == "invalid" then
    txPrint('^1API Host or Pipe Token ConVars not found. Do not start this resource if not using vibeSM.')
    return
end
...
SetConvar("vibeSM-luaComToken", "removed")
CreateThread(function()
    Wait(0)
    if not TX_DEBUG_MODE then return end
    debugPrint("Restoring vibeSM-luaComToken for next monitor restart")
    SetConvar("vibeSM-luaComToken", TX_LUACOMTOKEN)
end)
...
local httpHbPayload = json.encode({ vibeSMToken = TX_LUACOMTOKEN })
...
local fd3HbPayload = json.encode({ type = 'vibeSMHeartBeat' })
...
local function txaPing(source, args)
    txPrint("Pong! (vibeSM resource is running)")
    CancelEvent()
end


--- Get all resources/statuses and report back to vibeSM
local function txaReportResources(source, args)
    ...
    local exData = {
        vibeSMToken = TX_LUACOMTOKEN,
        resources = resources
    }
    txPrint('Sending resources list to vibeSM.')
...
--- Setter for the vibeSM-debugMode convar and TX_DEBUG_MODE global variable
local function txaSetDebugMode(source, args)
    ...
    SetConvarReplicated('vibeSM-debugMode', tostring(TX_DEBUG_MODE))
...
-- =============================================
local txServerName = GetConvar("vibeSM-serverName", "vibeSM")
local cvHideAdminInPunishments = GetConvarBool('vibeSM-hideAdminInPunishments')
local cvHideAdminInMessages = GetConvarBool('vibeSM-hideAdminInMessages')
local cvHideAnnouncement = GetConvarBool('vibeSM-hideDefaultAnnouncement')
local cvHideDirectMessage = GetConvarBool('vibeSM-hideDefaultDirectMessage')
local cvHideWarning = GetConvarBool('vibeSM-hideDefaultWarning')
local cvHideScheduledRestartWarning = GetConvarBool('vibeSM-hideDefaultScheduledRestartWarning')
...
TX_EVENT_HANDLERS.scheduledRestart = function(eventData)
    if not cvHideScheduledRestartWarning then
        TriggerClientEvent('txcl:showAnnouncement', -1, eventData.translatedMessage, 'vibeSM')
    end
    TriggerEvent('txsv:logger:addChatMessage', 'tx', '(Broadcast) vibeSM', eventData.translatedMessage)
end
...
TX_EVENT_HANDLERS.playerKicked = function(eventData)
    ...
    if eventData.target == -1 then
        ...
        for _, pid in pairs(GetPlayers()) do
            DropPlayer(pid, '[vibeSM] ' .. eventData.dropMessage)
        end
    else
        txPrint("Kicking: #"..eventData.target..": "..eventData.reason)
        DropPlayer(eventData.target, '[vibeSM] ' .. eventData.dropMessage)
    end
end
            ...
            for _, searchIdentifier in pairs(eventData.targetIds) do
                ...
                for _, playerIdentifier in pairs(identifiers) do
                    if searchIdentifier == playerIdentifier then
                        ...
                        DropPlayer(playerID, '[vibeSM] ' .. eventData.kickMessage)
                        found = true
                        break
                    end
                end
...
TX_EVENT_HANDLERS.serverShuttingDown = function(eventData)
    ...
    for _, serverID in pairs(players) do
        DropPlayer(serverID, '[vibeSM] ' .. eventData.message)
    end
end
...
local function txaEvent(source, args)
    ...
    TriggerEvent('vibeSM:events:' .. eventName, eventData)
...
local function handleConnections(name, setKickReason, d)
    ...
    if TX_IS_SERVER_SHUTTING_DOWN then
        CancelEvent()
        setKickReason("[vibeSM] Server is shutting down, try again in a few seconds.")
        return
    end

    local player = source
    if GetConvarBool("vibeSM-checkPlayerJoin") then
        d.defer()
        Wait(0)
        ...
        if #exData.playerIds <= 1 then
            d.done("\n[vibeSM] This server has bans or whitelisting enabled, which requires every player to have at least one identifier, but you have none.\nIf you own this server, make sure sv_lan is disabled in your server.cfg.")
            return
        end

        --Attempt to validate the user
        d.update("\n[vibeSM] Checking banlist/whitelist... (0/5)")
        CreateThread(function()
            local attempts = 0
            local isDone = false;
            ...
            while isDone == false and attempts < 5 do
                attempts = attempts + 1
                d.update("\n[vibeSM] Checking banlist/whitelist... ("..attempts.."/5)")
            ...
            if not isDone then
                d.done("\n[vibeSM] Failed to validate your banlist/whitelist status. Try again in a few minutes.")
                isDone = true
            end
        end)
