...
if not useLatentEvents then
  txPrint('^3WARNING: Latent events are disabled. If you have issues using the vibeSM in-game menu, please enable them by setting sv_enableNetEventReassembly to true in your server.cfg.')
end
...
RegisterNetEvent('txsv:webpipe:req', function(callbackId, method, path, headers, body)
  ...
  headers['X-VibeSM-Token'] = TX_LUACOMTOKEN
  headers['X-VibeSM-Identifiers'] = table.concat(GetPlayerIdentifiers(s), ',')
