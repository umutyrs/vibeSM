...
local function handleAuthFail(src, reason)
    ...
    TriggerEvent('vibeSM:events:adminAuth', {
        netid = src,
        isAdmin = false,
    })
end
...
RegisterNetEvent('txsv:checkIfAdmin', function()
    ...
    local headers = {
        ...
        ['X-VibeSM-Token'] = TX_LUACOMTOKEN,
        ['X-VibeSM-Identifiers'] = table.concat(GetPlayerIdentifiers(src), ',')
    }
...
AddEventHandler('vibeSM:events:adminsUpdated', function(onlineAdminIDs)
