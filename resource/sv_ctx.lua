...
local function syncServerCtx()
  ...
  -- https://keycode.info/
  local switchPageKey = GetConvar('vibeSM-menuPageKey', 'Tab')
  ServerCtxObj.switchPageKey = switchPageKey

  local alignRight = GetConvarBool('vibeSM-menuAlignRight')
  ServerCtxObj.alignRight = alignRight

  local vibeSMVersion = GetConvar('vibeSM-version', '0.0.0')
  ServerCtxObj.vibeSMVersion = vibeSMVersion
  ...
  -- Custom locale
  local vibeSMLocale = GetConvar('vibeSM-locale', 'en')
  ServerCtxObj.locale = vibeSMLocale
  if vibeSMLocale == 'custom' then
    ServerCtxObj.localeData = getCustomLocaleData()
  else
    ServerCtxObj.localeData = false
  end

  local announceNotiPos = GetConvar('vibeSM-menuAnnounceNotiPos', 'top-center')
  ...
  GlobalState.vibeSMServerCtx = ServerCtxObj
...
-- Everytime the vibeSM convars are changed this event will fire
-- Therefore, lets update global state with that.
AddEventHandler('vibeSM:events:configChanged', function()
