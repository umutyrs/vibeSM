...
export default async function Intercom(ctx: InitializedCtx) {
    ...
    postData.vibeSMToken = true;

    //Delegate to the specific scope functions
    if (scope == 'monitor') {
        try {
            vibeCore.fxMonitor.handleHeartBeat('http');
            return ctx.send(vibeCore.metrics.txRuntime.currHbData);
        } catch (error) {
            return ctx.send({
        ...
        vibeCore.fxResources.tmpUpdateResourceList(postData.resources);
    } else {
        return ctx.send({
            type: 'danger',
