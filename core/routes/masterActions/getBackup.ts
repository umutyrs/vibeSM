...
export default async function MasterActionsGet(ctx: AuthedCtx) {
    ...
    if (!ctx.txVars.isWebInterface) {
        return ctx.utils.render('main/message', { message: 'This functionality cannot be used by the in-game menu, please use the web version of vibeSM.' });
    }

    const dbPath = `${vibeEnv.profilePath}/data/playersDB.json`;
    let readFile;
    try {
