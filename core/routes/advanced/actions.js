...
export default async function AdvancedActions(ctx) {
    ...
    if (action == 'change_verbosity') {
        ...
        const message = JSON.stringify(vibeCore.fxPlayerlist.getPlayerList(), null, 2);
        return ctx.send({ type: 'success', message });
    } else if (action == 'show_db') {
        const dbo = vibeCore.database.getDboRef();
        console.dir(dbo);
        return ctx.send({ type: 'success', message: JSON.stringify(dbo, null, 2) });
    } else if (action == 'show_log') {
        return ctx.send({ type: 'success', message: JSON.stringify(vibeCore.logger.server.getRecentBuffer(), null, 2) });
        ...
        vibeCore.fxRunner.updateMutableConvars();
        return ctx.send({ refresh: true });
    } else if (action == 'reauthLast10Players') {
        // force refresh the admin status of the last 10 players to join
        const lastPlayers = vibeCore.fxPlayerlist.getPlayerList().map((p) => p.netid).slice(-10);
        vibeCore.fxRunner.sendEvent('adminsUpdated', lastPlayers);
        return ctx.send({ type: 'success', message: `refreshed: ${JSON.stringify(lastPlayers)}` });
    } else if (action == 'getLoggerErrors') {
        const outData = {
            admin: vibeCore.logger.admin.lrLastError,
            fxserver: vibeCore.logger.fxserver.lrLastError,
            server: vibeCore.logger.server.lrLastError,
        };
        ...
        const setCmdResult = vibeCore.fxRunner.sendCommand(
            'set',
            [
                'vibeSM-luaComToken',
                vibeCore.webServer.luaComToken,
            ],
            SYM_SYSTEM_AUTHOR
        );
        if (!setCmdResult) {
        ...
        const ensureCmdResult = vibeCore.fxRunner.sendCommand(
            'ensure',
            ['monitor'],
            SYM_SYSTEM_AUTHOR
        );
        ...
        const category = vibeCore.metrics.playerDrop.handlePlayerDrop(reason);
        ...
        try {
            ...
            const storedKeysChanges = vibeCore.configStore.saveConfigs(configUpdate, ctx.admin.name);
            const outParts = [
                'Keys Updated: ' + JSON.stringify(storedKeysChanges ?? 'not set', null, 2),
                '-'.repeat(16),
                'Stored:' + JSON.stringify(vibeCore.configStore.getStoredConfig(), null, 2),
            ];
        ...
        const message = JSON.stringify(vibeCore.fxRunner.history, null, 2)
        return ctx.send({ type: 'success', message });
