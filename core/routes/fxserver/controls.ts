...
export default async function FXServerControls(ctx: AuthedCtx) {
    ...
    if (store && store.serverId && store.serverId !== 'primary') {
        const { controlServer } = require('../multiHosting');
        ctx.params.id = store.serverId;
    ...
    if (action === 'restart') {
        ...
        const respawnDelay = vibeCore.fxRunner.restartSpawnDelay;
        if (respawnDelay.ms > 10_000) {
            vibeCore.fxRunner.restartServer('admin request', ctx.admin.name).catch((e) => { });
            ...
            const restartError = await vibeCore.fxRunner.restartServer('admin request', ctx.admin.name);
        ...
        await vibeCore.fxRunner.killServer('admin request', ctx.admin.name, false);
        ...
        const spawnError = await vibeCore.fxRunner.spawnServer(true);
        if (spawnError !== null) {
