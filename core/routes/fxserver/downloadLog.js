...
export default async function FXServerDownloadLog(ctx) {
    ...
    try {
        ...
        // readFile = fs.createReadStream(vibeCore.logger.fxserver.activeFilePath);
        readFile = fs.readFileSync(vibeCore.logger.fxserver.activeFilePath);
    } catch (error) {
        console.error(`Could not read log file ${vibeCore.logger.fxserver.activeFilePath}.`);
    }
