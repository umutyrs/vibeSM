...
async function handleWarning(ctx: AuthedCtx, player: PlayerClass): Promise<GenericApiResp> {
    ...
    let actionId;
    try {
        actionId = vibeCore.database.actions.registerWarn(
            allIds,
            ctx.admin.name,
            reason,
            player.displayName,
    ...
    // Dispatch `vibeSM:events:playerWarned`
    const eventSent = vibeCore.fxRunner.sendEvent('playerWarned', {
...
async function handleBan(ctx: AuthedCtx, player: PlayerClass): Promise<GenericApiResp> {
    ...
    let actionId;
    try {
        actionId = vibeCore.database.actions.registerBan(
            allIds,
            ctx.admin.name,
            reason,
            expiration,
    ...
    const tOptions: any = {
        author: vibeCore.adminStore.getAdminPublicName(ctx.admin.name, 'punishment'),
        reason: reason,
    };
    if (expiration !== false && duration) {
        durationTranslated = vibeCore.translator.tDuration(
            duration * 1000,
            { units: ['d', 'h'] },
        );
        tOptions.expiration = durationTranslated;
        kickMessage = vibeCore.translator.t('ban_messages.kick_temporary', tOptions);
    } else {
        durationTranslated = null;
        kickMessage = vibeCore.translator.t('ban_messages.kick_permanent', tOptions);
    }

    // Dispatch `vibeSM:events:playerBanned`
    const eventSent = vibeCore.fxRunner.sendEvent('playerBanned', {
        author: ctx.admin.name,
        reason,
        actionId,
        expiration,
...
async function handleSetWhitelist(ctx: AuthedCtx, player: PlayerClass): Promise<GenericApiResp> {
    ...
    try {
        ...
        // Dispatch `vibeSM:events:whitelistPlayer`
        vibeCore.fxRunner.sendEvent('whitelistPlayer', {
...
async function handleDirectMessage(ctx: AuthedCtx, player: PlayerClass): Promise<GenericApiResp> {
    ...
    //Validating server & player
    if (!vibeCore.fxRunner.child?.isAlive) {
    ...
    try {
        ...
        // Dispatch `vibeSM:events:playerDirectMessage`
        vibeCore.fxRunner.sendEvent('playerDirectMessage', {
            target: player.netid,
            author: ctx.admin.name,
            message,
        });
...
async function handleKick(ctx: AuthedCtx, player: PlayerClass): Promise<GenericApiResp> {
    ...
    const kickReason = ctx.request.body.reason.trim() || vibeCore.translator.t('kick_messages.unknown_reason');
    ...
    try {
        ...
        const dropMessage = vibeCore.translator.t(
            'kick_messages.player',
            { reason: kickReason }
        );

        // Dispatch `vibeSM:events:playerKicked`
        vibeCore.fxRunner.sendEvent('playerKicked', {
            target: player.netid,
            author: ctx.admin.name,
            reason: kickReason,
            dropMessage,
