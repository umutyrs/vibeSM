...
export default async function PlayerCheckJoin(ctx: InitializedCtx) {
    ...
    if (
        !vibeConfig.banlist.enabled
        && (vibeConfig.whitelist.mode === 'disabled' || vibeConfig.whitelist.mode === 'external')
    ) {
...
function checkBan(
    ...
    const activeBans = vibeCore.database.actions.findMany(validIdsArray, validHwidsArray, filter);
    if (activeBans.length) {
        const ban = activeBans[0];

        //Translation keys
        const textKeys = {
            title_permanent: vibeCore.translator.t('ban_messages.reject.title_permanent'),
            title_temporary: vibeCore.translator.t('ban_messages.reject.title_temporary'),
            label_expiration: vibeCore.translator.t('ban_messages.reject.label_expiration'),
            label_date: vibeCore.translator.t('ban_messages.reject.label_date'),
            label_author: vibeCore.translator.t('ban_messages.reject.label_author'),
            label_reason: vibeCore.translator.t('ban_messages.reject.label_reason'),
            label_id: vibeCore.translator.t('ban_messages.reject.label_id'),
            note_multiple_bans: vibeCore.translator.t('ban_messages.reject.note_multiple_bans'),
            note_diff_license: vibeCore.translator.t('ban_messages.reject.note_diff_license'),
        };
        ...
        let expLine = '';
        if (ban.expiration) {
            const duration = vibeCore.translator.tDuration(
        ...
        const banDate = new Date(ban.timestamp * 1000).toLocaleString(
            vibeCore.translator.canonical,
            { dateStyle: 'medium', timeStyle: 'medium' }
        )

        //Ban author
        let authorLine = '';
        if (!vibeConfig.gameFeatures.hideAdminInPunishments) {
        ...
        const reason = rejectMessageTemplate(
            ...
            ${authorLine}
            ${prepCustomMessage(vibeConfig.banlist.rejectionMessage)}
...
async function checkAdminOnlyMode(
    ...
    const textKeys = {
        mode_title: vibeCore.translator.t('whitelist_messages.admin_only.mode_title'),
        insufficient_ids: vibeCore.translator.t('whitelist_messages.admin_only.insufficient_ids'),
        deny_message: vibeCore.translator.t('whitelist_messages.admin_only.deny_message'),
    };
    ...
    //Looking for admin
    const admin = vibeCore.adminStore.getAdminByIdentifiers(validIdsArray);
    ...
    const reason = rejectMessageTemplate(
        ...
        ${prepCustomMessage(vibeConfig.whitelist.rejectionMessage)}`
    );
    return { allow: false, reason };
}
...
async function checkDiscordMember(
    ...
    const guildname = `<guildname>${vibeCore.discordBot.guildName}</guildname>`;
    const textKeys = {
        mode_title: vibeCore.translator.t('whitelist_messages.guild_member.mode_title'),
        insufficient_ids: vibeCore.translator.t('whitelist_messages.guild_member.insufficient_ids'),
        deny_title: vibeCore.translator.t('whitelist_messages.guild_member.deny_title'),
        deny_message: vibeCore.translator.t('whitelist_messages.guild_member.deny_message', { guildname }),
    };
...
async function checkDiscordRoles(
    ...
    const guildname = `<guildname>${vibeCore.discordBot.guildName}</guildname>`;
    const textKeys = {
        mode_title: vibeCore.translator.t('whitelist_messages.guild_roles.mode_title'),
        insufficient_ids: vibeCore.translator.t('whitelist_messages.guild_roles.insufficient_ids'),
        deny_notmember_title: vibeCore.translator.t('whitelist_messages.guild_roles.deny_notmember_title'),
        deny_notmember_message: vibeCore.translator.t('whitelist_messages.guild_roles.deny_notmember_message', { guildname }),
        deny_noroles_title: vibeCore.translator.t('whitelist_messages.guild_roles.deny_noroles_title'),
        deny_noroles_message: vibeCore.translator.t('whitelist_messages.guild_roles.deny_noroles_message', { guildname }),
    };
    ...
    try {
        const { isMember, memberRoles } = await vibeCore.discordBot.resolveMemberRoles(validIdsObject.discord);
        if (isMember) {
            const matchingRole = vibeConfig.whitelist.discordRoles
...
async function checkApprovedLicense(
    ...
    const textKeys = {
        mode_title: vibeCore.translator.t('whitelist_messages.approved_license.mode_title'),
        insufficient_ids: vibeCore.translator.t('whitelist_messages.approved_license.insufficient_ids'),
        deny_title: vibeCore.translator.t('whitelist_messages.approved_license.deny_title'),
        request_id_label: vibeCore.translator.t('whitelist_messages.approved_license.request_id_label'),
    };
    ...
    const approvals = vibeCore.database.whitelist.findManyApprovals(allIdsFilter);
    if (approvals.length) {
        //update or register player
        if (typeof player !== 'undefined' && player.license) {
            player.setWhitelist(true);
        } else {
            vibeCore.database.players.register({
        ...
        vibeCore.database.whitelist.removeManyApprovals(allIdsFilter);
        vibeCore.database.whitelist.removeManyRequests({ license: validIdsObject.license });
    ...
    if (validIdsObject.discord && vibeCore.discordBot.isClientReady) {
        try {
            const { tag, avatar } = await vibeCore.discordBot.resolveMemberProfile(validIdsObject.discord);
            discordTag = tag;
    ...
    const requests = vibeCore.database.whitelist.findManyRequests({ license: validIdsObject.license });
    if (requests.length) {
        wlRequestId = requests[0].id; //just getting the first
        vibeCore.database.whitelist.updateRequest(validIdsObject.license, {
        ...
        wlRequestId = vibeCore.database.whitelist.registerRequest({
            license: validIdsObject.license,
            playerDisplayName: displayName,
            playerPureName: pureName,
            discordTag,
        ...
        vibeCore.fxRunner.sendEvent('whitelistRequest', {
            action: 'requested',
