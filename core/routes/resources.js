...
const getResourceSubPath = (resPath) => {
    ...
    let serverDataPathArr = breakPath(`${vibeConfig.server.dataPath}/resources`);
...
export default async function Resources(ctx) {
    ...
    - Make sure you are not running the FXServer outside of vibeSM. <br>
    ...
    const cmdSuccess = vibeCore.fxRunner.sendCommand('txaReportResources', [], SYM_SYSTEM_AUTHOR);
    if (!cmdSuccess) {
    ...
    const tList = new Promise((resolve, reject) => {
        tListTimer = setInterval(() => {
            if (
                vibeCore.fxResources.resourceReport
                && (new Date() - vibeCore.fxResources.resourceReport.ts) <= 5000
                && Array.isArray(vibeCore.fxResources.resourceReport.resources)
            ) {
                clearTimeout(tListTimer);
                clearTimeout(tErrorTimer);
                const resGroups = processResources(vibeCore.fxResources.resourceReport.resources);
                const renderData = {
                    headerTitle: 'Resources',
                    resGroupsJS: JSON.stringify(resGroups),
                    resGroups,
                    disableActions: (ctx.admin.hasPermission('commands.resources')) ? '' : 'disabled',
                    hasFileEditorPermission: ctx.admin.hasPermission('resources.file_editor'),
                };
