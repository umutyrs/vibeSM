...
export default async function AuthAddMasterSave(ctx: InitializedCtx) {
    ...
    try {
        ...
        if (userInfo.picture) {
            vibeCore.cacheStore.set(`admin:picture:${userInfo.name}`, userInfo.picture);
        }
