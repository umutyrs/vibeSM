...
export default async function AuthProviderCallback(ctx: InitializedCtx) {
    ...
    if(!fivemIdentifier){
        return ctx.send<ApiOauthCallbackResp>({
            ...
            errorMessage: `Could not extract the user identifier from the URL below. Please report this to the vibeSM dev team.\n${userInfo.nameid.toString()}`,
        });
    }

    //Check & Login user
    try {
        const vaultAdmin = vibeCore.adminStore.getAdminByIdentifiers([fivemIdentifier]);
        if (!vaultAdmin) {
        ...
        if (userInfo.picture) {
            vibeCore.cacheStore.set(`admin:picture:${vaultAdmin.name}`, userInfo.picture);
        }
        ...
        vibeCore.metrics.txRuntime.loginMethods.count('citizenfx');
