...
export const getOauthRedirectUrl = (ctx: InitializedCtx, purpose: 'login' | 'addMaster', origin: string) => {
    ...
    const idmsAuthUrl = vibeCore.adminStore.providers.citizenfx.getAuthURL(
        callbackUrl,
...
export const handleOauthCallback = async (ctx: InitializedCtx, redirectUri: string): Promise<ApiOauthCallbackErrorResp | UserInfoType> => {
    ...
    let tokenSet;
    try {
        tokenSet = await vibeCore.adminStore.providers.citizenfx.processCallback(
    ...
    //Get userinfo
    try {
        return await vibeCore.adminStore.providers.citizenfx.getUserInfo(tokenSet.access_token);
    } catch (error) {
