...
async function handleBandIds(ctx: AuthedCtx): Promise<GenericApiOkResp> {
    ...
    try {
        ...
        vibeCore.fxRunner.sendEvent('playerBanned', {
            author: ctx.admin.name,
            reason,
            actionId,
            expiration,
...
async function handleRevokeAction(ctx: AuthedCtx): Promise<GenericApiOkResp> {
    ...
    let action;
    try {
        action = vibeCore.database.actions.revoke(actionId, ctx.admin.name, perms) as DatabaseActionType;
    ...
    // Dispatch `vibeSM:events:actionRevoked`
    try {
        vibeCore.fxRunner.sendEvent('actionRevoked', {
            actionId: action.id,
