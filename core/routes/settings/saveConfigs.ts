...
const handleFxserverCard: CardHandler = async (inputConfig, sendTypedResp) => {
    ...
    //Validating CFG Path
    let cfgPath = vibeConfig.server.cfgPath;
    ...
    if (typeof inputConfig.discordBot.embedJson === 'string' || typeof inputConfig.discordBot.embedConfigJson === 'string') {
        try {
            generateStatusMessage(
                inputConfig.discordBot.embedJson as string | undefined ?? vibeConfig.discordBot.embedJson,
                inputConfig.discordBot.embedConfigJson as string | undefined ?? vibeConfig.discordBot.embedConfigJson,
            );
        } catch (error) {
            return sendTypedResp({
                type: 'error',
