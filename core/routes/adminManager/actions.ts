...
async function handleAdd(ctx: AuthedCtx) {
    ...
    try {
        await vibeCore.adminStore.addAdmin(name, citizenfxData, discordData, password, permissions);
...
async function handleEdit(ctx: AuthedCtx) {
    ...
    const admin = vibeCore.adminStore.getAdminByName(name);
    ...
    //Add admin and give output
    try {
        await vibeCore.adminStore.editAdmin(name, null, citizenfxData, discordData, permissions);
...
async function handleDelete(ctx: AuthedCtx) {
    ...
    try {
        await vibeCore.adminStore.deleteAdmin(name);
