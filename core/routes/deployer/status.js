...
export default async function DeployerStatus(ctx) {
    ...
    //Prepare data
    const outData = {
        progress: vibeManager.deployer.progress,
        log: vibeManager.deployer.getDeployerLog(),
    };
    if (vibeManager.deployer.step == 'configure') {
        outData.status = 'done';
    } else if (vibeManager.deployer.deployFailed) {
