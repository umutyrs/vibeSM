...
async function handleConfirmRecipe(ctx) {
    ...
    try {
        ...
        await vibeManager.deployer.confirmRecipe(userEditedRecipe);
    } catch (error) {
...
async function handleSetVariables(ctx) {
    ...
    if (
        vibeManager.deployer.recipe.steamRequired
    ...
    if (typeof userVars.dbDelete !== 'undefined') {
        ...
        try {
            ...
            if (error?.code === 'ECONNREFUSED') {
                let specificError = (vibeEnv.isWindows)
    ...
    userVars.maxClients = (vibeHostConfig.forceMaxClients) ? vibeHostConfig.forceMaxClients : 48;
    if (vibeHostConfig.netInterface || vibeHostConfig.fxsPort) {
        const comment = `# ${vibeHostConfig.sourceName}: do not modify!`;
        const endpointIface = vibeHostConfig.netInterface ?? '0.0.0.0';
        const endpointPort = vibeHostConfig.fxsPort ?? 30120;
    ...
    const admin = vibeCore.adminStore.getAdminByName(ctx.admin.name);
    ...
    try {
        ...
        vibeManager.deployer.start(userVars);
    } catch (error) {
        return ctx.send({ type: 'danger', message: error.message });
    }
...
async function handleSaveConfig(ctx) {
    ...
    const cfgFilePath = path.join(vibeManager.deployer.deployPath, 'server.cfg');
    vibeCore.cacheStore.set('deployer:recipe', vibeManager.deployer?.recipe?.name ?? 'unknown');

    //Validating config contents + saving file and backup
    try {
        const result = await validateModifyServerConfig(serverCFG, cfgFilePath, vibeManager.deployer.deployPath);
        if (result.errors) {
            return ctx.send({
                type: 'danger',
                success: false,
    ...
    if (typeof vibeManager.deployer?.recipe?.onesync === 'string' && vibeManager.deployer.recipe.onesync.length) {
    ...
    if (!vibeCore.fxRunner.isIdle) {
        ...
        await vibeCore.fxRunner.killServer('new server deployed', ctx.admin.name, true);
    }

    //Starting server
    const spawnError = await vibeCore.fxRunner.spawnServer(false);
    if (spawnError !== null) {
        return ctx.send({
            type: 'danger',
            markdown: true,
