...
async function handleSaveLocal(ctx) {
    ...
    //Refreshing config
    vibeCore.cacheStore.set('deployer:recipe', 'none');
    ...
    if (!vibeCore.fxRunner.isIdle) {
        ...
        await vibeCore.fxRunner.killServer('new server set up', ctx.admin.name, true);
    }
...
async function handleSaveDeployerImport(ctx) {
    ...
    try {
        vibeManager.startDeployer(recipeText, deploymentID, targetPath, isTrustedSource, {serverName});
...
async function handleSaveDeployerCustom(ctx) {
    ...
    try {
        vibeManager.startDeployer(false, deploymentID, targetPath, false, customMetaData);
        vibeCore.webServer.webSocket.pushRefresh('status');
    } catch (error) {
        return ctx.send({success: false, message: error.message});
    }
