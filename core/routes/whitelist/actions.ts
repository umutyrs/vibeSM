...
async function handleApprovals(ctx: AuthedCtx, action: any): Promise<GenericApiResp> {
    ...
    if (action === 'add') {
        ...
        if (idType === 'discord') {
            try {
                const { tag, avatar } = await vibeCore.discordBot.resolveMemberProfile(idValue);
                playerName = tag;
                playerAvatar = avatar;
            } catch (error) { }
        }
        ...
        try {
            vibeCore.database.whitelist.registerApproval({
            ...
            vibeCore.fxRunner.sendEvent('whitelistPreApproval', {
                action: 'added',
                identifier: idlowerCased,
                playerName,
        ...
        try {
            vibeCore.database.whitelist.removeManyApprovals({ identifier: idlowerCased });
            vibeCore.fxRunner.sendEvent('whitelistPreApproval', {
                action: 'removed',
                identifier: idlowerCased,
                adminName: ctx.admin.name,
...
async function handleRequests(ctx: AuthedCtx, action: any): Promise<GenericApiResp> {
    ...
    if (action === 'deny_all') {
        ...
        try {
            ...
            vibeCore.database.whitelist.removeManyRequests(filter);
    ...
    if (action === 'approve') {
        //Find request
        const requests = vibeCore.database.whitelist.findManyRequests({ id: reqId });
        if (!requests.length) {
        ...
        try {
            vibeCore.database.whitelist.removeManyRequests({ id: reqId });
        } catch (error) {
        ...
        try {
            const requests = vibeCore.database.whitelist.removeManyRequests({ id: reqId });
            if(requests.length){
                const req = requests[0]; //just getting the first
                vibeCore.fxRunner.sendEvent('whitelistRequest', {
                    action: 'denied',
