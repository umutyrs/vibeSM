...
import { txDevEnv, vibeEnv, vibeHostConfig } from '@core/globalData';
...
const getOSMessage = async () => {
    const serverMessage = [
        `To be able to access vibeSM from the internet open port ${vibeHostConfig.txaPort}`,
    ...
    if (vibeEnv.displayAds) {
        winWorkstationMessage.push('We recommend renting a server from ' + chalk.inverse(' https://zap-hosting.com/vibeSM ') + '.');
    }
...
const awaitHttp = new Promise((resolve, reject) => {
    ...
    const check = () => {
        counter++;
        if (vibeCore.webServer && vibeCore.webServer.isListening && vibeCore.webServer.isServing) {
            ...
            console.warn('The WebServer is taking too long to start:', {
                module: !!vibeCore.webServer,
                listening: vibeCore?.webServer?.isListening,
                serving: vibeCore?.webServer?.isServing,
            });
        }
    };
...
const awaitMasterPin = new Promise((resolve, reject) => {
    ...
    const check = () => {
        counter++;
        if (vibeCore.adminStore && vibeCore.adminStore.admins !== null) {
            clearInterval(interval);
            const pin = (vibeCore.adminStore.admins === false) ? vibeCore.adminStore.addMasterPin : false;
            resolve(pin);
        } else if (counter == tickLimit) {
            clearInterval(interval);
            interval = setInterval(check, 2500);
            ...
            console.warn('The AdminStore is taking too long to start:', {
                module: !!vibeCore.adminStore,
                admins: vibeCore?.adminStore?.admins === null ? 'null' : 'not null',
            });
        }
    };
    interval = setInterval(check, 150);
...
const awaitDatabase = new Promise((resolve, reject) => {
    ...
    const check = () => {
        counter++;
        if (vibeCore.database && vibeCore.database.isReady) {
            clearInterval(interval);
            resolve(true);
        } else if (counter == tickLimit) {
            clearInterval(interval);
            ...
            console.warn('The Database is taking too long to start:', {
                module: !!vibeCore.database,
                ready: !!vibeCore?.database?.isReady,
            });
        }
    };
    interval = setInterval(check, 150);
...
export const startReadyWatcher = async (cb: () => void) => {
    ...
    //Addresses
    let detectedUrls;
    if (vibeHostConfig.netInterface && vibeHostConfig.netInterface !== '0.0.0.0') {
        detectedUrls = [vibeHostConfig.netInterface];
    } else {
        detectedUrls = [
            (vibeEnv.isWindows) ? 'localhost' : 'your-public-ip',
        ];
    ...
    const bannerUrls = vibeHostConfig.txaUrl
        ? [vibeHostConfig.txaUrl]
        : detectedUrls.map((addr) => `http://${addr}:${vibeHostConfig.txaPort}/`);
    ...
    if (!txDevEnv.ENABLED && !vibeHostConfig.netInterface && 'value' in msgRes && msgRes.value) {
        console.multiline(msgRes.value, chalk.bgBlue);
    }

    //Opening page
    if (vibeEnv.isWindows && adminMasterPin && bannerUrls[0]) {
