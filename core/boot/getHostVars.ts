...
/**
 * Schemas for the VIBEHOST_ env variables
 */
export const hostEnvVarSchemas = {
    ...
    //Networking
    VIBE_URL: z.string().url(),
    VIBE_PORT: z.coerce.number().int().positive().refine(
        (val) => val !== 30120,
        'VIBE_PORT cannot be 30120'
    ),
...
/**
 * Parses the VIBEHOST_ env variables
 */
export const getHostVars = () => {
    ...
    for (const partialKey of Object.keys(hostEnvVarSchemas)) {
        ...
        const fullKey = `VIBEHOST_` + partialKey;
