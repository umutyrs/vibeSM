...
/**
 * Gets & parses the vibeSMZapConfig.json variables
 */
export const getZapVars = (zapCfgFilePath: string): ZapConfigVars | undefined => {
    if (!fs.existsSync(zapCfgFilePath)) return;
    console.warn(`WARNING: The 'vibeSMZapConfig.json' file has been deprecated and this feature will be removed in the next update.`);
    console.warn(`         Please use the 'VIBEHOST_' environment variables instead.`);
    console.warn(`         For more information: https://aka.cfx.re/vibesm-env-config.`);
    ...
    const zapVars: ZapConfigVars = {
        ...
        vibeSMPort: allowType('number', cfgFileData.vibeSMPort),
