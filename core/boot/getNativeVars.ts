...
export const getNativeVars = (ignoreDeprecatedConfigs: boolean) => {
    ...
    const vibeSMProfile = getConvarString('serverProfile');
    if (vibeSMProfile) {
        console.warn(`WARNING: The 'serverProfile' ConVar is deprecated and will be removed in a future update.`);
        console.warn(`         To create multiple servers, set up a different VIBEHOST_DATA_PATH instead.`);
        anyWarnSent = true;
    }

    //Convars replaced by VIBEHOST_* env vars
    let txDataPath, vibeSMPort, vibeSMInterface;
    if (!ignoreDeprecatedConfigs) {
        txDataPath = getConvarString('txDataPath');
        if (txDataPath) {
            replacedConvarWarning('txDataPath', 'VIBEHOST_DATA_PATH');
            ...
            if (!path.isAbsolute(txDataPath)) {
                ...
                console.error(`         VIBEHOST_DATA_PATH=${txDataPath}`);
            }
        }
        vibeSMPort = getConvarString('vibeSMPort');
        if (vibeSMPort) replacedConvarWarning('vibeSMPort', 'VIBEHOST_VIBE_PORT');
        vibeSMInterface = getConvarString('vibeSMInterface');
        if (vibeSMInterface) replacedConvarWarning('vibeSMInterface', 'VIBEHOST_INTERFACE');
    }

    if (anyWarnSent) {
        console.warn(`WARNING: For more information: https://aka.cfx.re/vibesm-env-config`);
    }
