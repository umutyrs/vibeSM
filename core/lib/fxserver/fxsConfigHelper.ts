...
class FilesInfoList {
    ...
    toMarkdown(hasHostConfig = false) {
        ...
        if (hasHostConfig) {
            ...
            msgLines.push(`**Some of the configuration above is controlled by ${vibeHostConfig.sourceName}.**`);
        }
...
const validateCommands = async (parsedCommands: (ExecRecursionError | Command)[]) => {
    ...
    const requiredGameName = vibeHostConfig.forceGameName
        ? vibeHostConfig.forceGameName === 'fivem' ? 'gta5' : 'rdr3'
        : undefined;
    ...
    for (const cmd of parsedCommands) {
        ...
        //Check for start/stop/ensure vibeSM/vibeSMClient/monitor
        if (
            ['start', 'stop', 'ensure'].includes(cmd.command)
            && cmd.args.length >= 1
            && ['vibesm', 'vibesmclient', 'monitor'].includes(cmd.args[0].toLowerCase())
        ) {
            toCommentOut.add(
                cmd.file,
                cmd.line,
                'you MUST NOT start/stop/ensure vibesm resources.',
            );
            continue;
        }
        ...
        if (
            ...
            if (maxClients > vibeHostConfig.forceMaxClients) {
                hasHostConfigMessage = true;
                errors.add(
                    cmd.file,
                    cmd.line,
                    `your 'sv_maxclients' MUST be <= ${vibeHostConfig.forceMaxClients}.`
                );
                continue;
            }
        }
        ...
        if (cmd.isConvarSetterFor('onesync')) {
            toCommentOut.add(
                cmd.file,
                cmd.line,
                'onesync MUST only be set in the vibeSM settings page.',
            );
            continue;
        }
        ...
        if (cmd.command === 'endpoint_add_tcp' || cmd.command === 'endpoint_add_udp') {
            ...
            if (vibeHostConfig.netInterface && iface !== vibeHostConfig.netInterface) {
                hasHostConfigMessage = true;
                errors.add(
                    cmd.file,
                    cmd.line,
                    `the \`${cmd.command}\` interface MUST be \`${vibeHostConfig.netInterface}\`.`
                );
                continue;
            }
            ...
            if (port >= 40120 && port <= 40150) {
                errors.add(
                    cmd.file,
                    cmd.line,
                    `the \`${cmd.command}\` port \`${port}\` is dedicated for vibeSM and CAN NOT be used for FXServer.`
                );
                continue;
            }
            if (port === vibeHostConfig.txaPort) {
                errors.add(
                    cmd.file,
                    cmd.line,
                    `the \`${cmd.command}\` port \`${port}\` is being used by vibeSM and CAN NOT be used for FXServer at the same time.`
                );
                continue;
            }
            if (vibeHostConfig.fxsPort && port !== vibeHostConfig.fxsPort) {
                hasHostConfigMessage = true;
                errors.add(
                    cmd.file,
                    cmd.line,
                    `the \`${cmd.command}\` port MUST be \`${vibeHostConfig.fxsPort}\`.`
                );
                continue;
            }
    ...
    if (vibeHostConfig.forceGameName === 'redm' && detectedGameName !== 'rdr3') {
...
const getConnectEndpoint = (endpoints: EndpointsObjectType, hasEndpointCommand: boolean) => {
    if (!Object.keys(endpoints).length) {
        const instruction = hasEndpointCommand
            ...
            : 'Please'
        const suggestedPort = vibeHostConfig.fxsPort ?? 30120;
        const suggestedInterface = vibeHostConfig.netInterface ?? '0.0.0.0';
    ...
    for (const targetCfgPath in toCommentOut.store) {
        ...
        try {
            ...
            for (const [ln, reason] of actions) {
                ...
                cfgLines[ln - 1] = `## [vibeSM CFG validator]: ${reason}${fileEOL}# ${cfgLines[ln - 1]}`;
