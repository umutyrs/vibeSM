    ...
    try {
        ...
        Object.keys(processes).forEach((pid) => {
            ...
            if (currPidInt === txProcessId) {
                procName = 'vibeSM (inside FXserver)';
            ...
            procList.push({
                pid: currPidInt,
                ppid: (curr.ppid === txProcessId) ? `${txProcessId} (vibeSM)` : curr.ppid,
                name: procName,
                cpu: curr.cpu,
...
export const getFXServerData = async () => {
    ...
    try {
        ...
        return {
            ...
            versionMismatch: (ver.build !== vibeEnv.fxsVersion),
            ...
            vibeSMVersion: (infoData.vars && infoData.vars['vibeSM-version']) ? infoData.vars['vibeSM-version'] : '--',
            txAdminVersion: (infoData.vars && infoData.vars['vibeSM-version']) ? infoData.vars['vibeSM-version'] : '--',
        };
    } catch (error) {
...
export const getTxAdminData = async () => {
    const stats = vibeCore.metrics.txRuntime; //shortcut
    const memoryUsage = getHeapStatistics();

    let hostApiTokenState = 'not configured';
    if (vibeHostConfig.hostApiToken === 'disabled') {
        hostApiTokenState = 'disabled';
    } else if (vibeHostConfig.hostApiToken) {
        hostApiTokenState = 'configured';
    }

    const defaultFlags = Object.entries(vibeHostConfig.defaults).filter(([k, v]) => Boolean(v)).map(([k, v]) => k);
    return {
        //Stats
        uptime: msToDuration(process.uptime() * 1000),
        databaseFileSize: bytes(vibeCore.database.fileSize),
        vibeHostConfig: {
            ...vibeHostConfig,
        ...
        logger: {
            storageSize: (await vibeCore.logger.getStorageSize()).total,
            statusAdmin: vibeCore.logger.admin.getUsageStats(),
            statusFXServer: vibeCore.logger.fxserver.getUsageStats(),
            statusServer: vibeCore.logger.server.getUsageStats(),
        },
        memoryUsage: {
