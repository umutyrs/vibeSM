...
export class BasePlayer {
    ...
    protected mutateDbData(srcData: object) {
        ...
        this.dbData = vibeCore.database.players.update(this.license, srcData, this.uniqueId);
    }
    ...
    getHistory() {
        ...
        return vibeCore.database.actions.findMany(
    ...
    setWhitelist(enabled: boolean) {
        ...
        vibeCore.database.whitelist.removeManyRequests({ license: this.license });
    }
}
...
export class ServerPlayer extends BasePlayer {
    ...
    #setupDatabaseData() {
        ...
        try {
            const dbPlayer = vibeCore.database.players.findOne(this.license);
            if (dbPlayer) {
                ...
                vibeCore.database.players.register(toRegister);
...
export class DatabasePlayer extends BasePlayer {
    ...
    constructor(license: string, srcPlayerData?: DatabasePlayerType) {
        ...
        if (srcPlayerData) {
            ...
            const foundData = vibeCore.database.players.findOne(license);
            if (!foundData) {
