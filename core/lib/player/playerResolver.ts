...
export default (mutex: any, netid: any, license: any) => {
    ...
    if(mutex === SYM_CURRENT_MUTEX){
        mutex = vibeCore.fxRunner.child?.mutex;
        if (!mutex) {
    ...
    if (typeof mutex === 'string' && typeof netid === 'number' && !isNaN(parsedNetid)) {
        hasMutex = true;
        if (mutex && mutex === vibeCore.fxRunner.child?.mutex) {
            //If the mutex is from the server currently online
            const player = vibeCore.fxPlayerlist.getPlayerById(netid);
            ...
            const found = vibeCore.fxPlayerlist.licenseCache.find(c => c[0] === searchRef);
    ...
    if (typeof searchLicense === 'string' && searchLicense.length) {
        const onlineMatches = vibeCore.fxPlayerlist.getOnlinePlayersByLicense(searchLicense);
