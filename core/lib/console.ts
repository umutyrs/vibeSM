...
//      which means in this file you cannot import anything from inside vibeSM to prevent cyclical dependencies
...
let _vibeSMVersion: string | undefined;
let _verboseFlag = false;

export const setConsoleEnvData = (
    vibeSMVersion: string,
    vibeSMResourcePath: string,
    isDevMode: boolean,
    isVerbose: boolean,
) => {
    _vibeSMVersion = vibeSMVersion;
    _verboseFlag = isVerbose;
    if (isDevMode) {
        sourceMapSupport.install();
        //for some reason when using sourcemap it ends up with core/core/
        stackPathAlias = {
            path: vibeSMResourcePath + '/core',
            alias: '@monitor',
        }
    } else {
        stackPathAlias = {
            path: vibeSMResourcePath,
            alias: '@monitor',
        }
    }
}
...
export const setTTYTitle = (title?: string) => {
    const txVers = _vibeSMVersion ? `vibeSM v${_vibeSMVersion}` : 'vibeSM';
    const out = title ? `${title} - vibeSM` : txVers;
