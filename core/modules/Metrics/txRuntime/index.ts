 ...
 * NOTE: the register functions don't throw because we rather break stats than vibeSM itself
 */
export default class TxRuntimeMetrics {
    ...
    /**
     * Processes general vibesm stuff to generate the HB data.
     * 
    ...
    async refreshHbData() {
        ...
        try {
            ...
            const statsData = {
                //Static
                providerName: vibeHostConfig.providerName,
                isZapHosting: vibeEnv.isZapHosting,
                isPterodactyl: vibeEnv.isPterodactyl,
                ...
                botCommands: vibeConfig.discordBot.enabled
                    ? this.botCommands
                    : false,
                menuCommands: vibeConfig.gameFeatures.menuEnabled
                    ? this.menuCommands
                    : false,
                banCheckTime: vibeConfig.banlist.enabled
                    ? this.banCheckTime
                    : false,
                whitelistCheckTime: vibeConfig.whitelist.mode !== 'disabled'
                ...
                //Settings & stuff
                adminCount: Array.isArray(vibeCore.adminStore.admins) ? vibeCore.adminStore.admins.length : 1,
                banCheckingEnabled: vibeConfig.banlist.enabled,
                whitelistMode: vibeConfig.whitelist.mode,
                recipeName: vibeCore.cacheStore.get('deployer:recipe') ?? 'not_in_cache',
                tmpConfigFlags: Object.entries(vibeConfig.gameFeatures)
                    .filter(([key, value]) => value)
                    .map(([key]) => key),

                //Processed stuff
                playerDb: vibeCore.database.stats.getDatabaseStats(),
                perfSummary: vibeCore.metrics.svRuntime.getServerPerfSummary(),
            };
