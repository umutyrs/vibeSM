...
import { txDevEnv, vibeEnv } from '@core/globalData';
...
export default class SvRuntimeMetrics {
    private readonly logFilePath = `${vibeEnv.profilePath}/data/${LOG_DATA_FILE_NAME}`;
    ...
    public logServerBoot(duration: number) {
        ...
        vibeCore.webServer.webSocket.pushRefresh('dashboard');
    ...
    private async collectStats() {
        //Precondition checks
        const monitorStatus = vibeCore.fxMonitor.status;
        ...
        if (!vibeCore.fxRunner.child?.isAlive) return;

        //Get performance data
        const netEndpoint = txDevEnv.EXT_STATS_HOST ?? vibeCore.fxRunner.child.netEndpoint;
        ...
        const [fetchRawPerfDataRes, fetchFxsMemoryRes] = await Promise.allSettled([
            ...
            fetchFxsMemory(vibeCore.fxRunner.child.pid),
        ]);
        ...
        vibeCore.metrics.txRuntime.perfCollectionTime.count(collectionTime.milliseconds);
        ...
        let playerCount = vibeCore.fxPlayerlist.onlineCount;
