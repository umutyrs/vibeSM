...
export default class AdminStore {
    constructor() {
        this.adminsFile = vibeHostConfig.dataSubPath('admins.json');
        ...
        //FIXME: maybe rename all_permissions to `administrator` (just like discord) or `super_admin` and rename the `Admins` page to `Users`. This fits better with how people use vibeSM as "mods" are not really admins
        this.registeredPermissions = {
            ...
            'settings.multihosting': 'Settings: Multi-Hosting',
            ...
            'resources.file_editor': 'File Editor: Read/Write resource files',
            'vibesm.log.view': 'View System Logs', //FIXME: rename to system.log.view
        ...
        if (!adminFileExists) {
            if (!vibeHostConfig.defaults.account) {
                this.addMasterPin = (Math.random() * 10000).toFixed().padStart(4, '0');
                this.admins = false;
            } else {
                const { username, fivemId, password } = vibeHostConfig.defaults.account;
                ...
                console.ok(`Created master account ${chalkInversePad(username)} with credentials provided by ${vibeHostConfig.sourceName}.`);
            }
        } else {
            this.loadAdminsFile();
    ...
    async checkAdminsFile() {
        ...
        try {
            ...
            if (this.adminsFileHash !== inboundHash) {
                console.warn('The admins.json file was modified or deleted by an external source, vibeSM will try to restore it.');
                restore();
            }
        } catch (error) {
    ...
    async loadAdminsFile() {
        ...
        const callError = (reason) => {
            ...
            if (reason === 'cannot read file') {
                details = ['This means the file  doesn\'t exist or vibeSM doesn\'t have permission to read it.'];
            } else {
                details = [
                    'This likely means the file got somehow corrupted.',
                    'You can try restoring it or you can delete it and let vibeSM create a new one.',
                ];
            }
    ...
    async refreshOnlineAdmins() {
        ...
        vibeCore.webServer.webSocket.reCheckAdminAuths().catch((e) => { });
        ...
        try {
            ...
            const playerList = vibeCore.fxPlayerlist.getPlayerList();
            const onlineIDs = playerList.filter((p) => {
                return p.ids.some((i) => adminIDs.includes(i));
            }).map((p) => p.netid);

            vibeCore.fxRunner.sendEvent('adminsUpdated', onlineIDs);
        } catch (error) {
    ...
    getAdminPublicName(name, purpose) {
        ...
        const replacer = vibeConfig.general.serverName ?? 'vibeSM';

        if (purpose === 'punishment') {
            return vibeConfig.gameFeatures.hideAdminInPunishments ? replacer : name;
        } else if (purpose === 'message') {
            return vibeConfig.gameFeatures.hideAdminInMessages ? replacer : name;
        } else {
