...
export default class UpdateChecker {
    ...
    /**
     * Check for vibeSM and FXServer updates
     */
    async checkChangelog() {
        ...
        //If vibeSM update, check for delay before printing
        if (updates.txa) {
            //Setup delay data
            const currTs = Date.now();
            let delayData: CachedDelayType;
            const rawCache = vibeCore.cacheStore.get(DELAY_CACHE_KEY);
            ...
            if (cachedData) {
                ...
                vibeCore.cacheStore.set(DELAY_CACHE_KEY, createCacheString(delayData));
            }

            //Get the delay
            const notifDelayDays = getUpdateRolloutDelay(
                updates.txa.semverDiff,
                vibeEnv.vibeVersion.includes('-'),
                delayData.diceRoll
            );
            ...
            if (currTs - delayData.ts >= notifDelayMs) {
                vibeCore.cacheStore.delete(DELAY_CACHE_KEY);
                this.txaUpdateData = {
                    version: updates.txa.version,
                    isImportant: updates.txa.isImportant,
                }
                if (updates.txa.isImportant) {
                    console.error('This version of vibeSM is outdated.');
                    console.error('Please update as soon as possible.');
                    console.error('For more information: https://discord.gg/uAmsGa2');
                } else {
                    console.warn('This version of vibeSM is outdated.');
                    console.warn('A patch (bug fix) update is available for vibeSM.');
        ...
        if (this.txaUpdateData || this.fxsUpdateData) {
            vibeCore.webServer.webSocket.pushEvent<UpdateAvailableEventType>('updateAvailable', {
                fxserver: this.fxsUpdateData,
                vibesm: this.txaUpdateData,
            });
        }
    }
};
...
/*
    ...
    Other relevant apis:
        ...
        https://api.github.com/repos/tabarra/vibeSM/releases (changelog in [].body)
