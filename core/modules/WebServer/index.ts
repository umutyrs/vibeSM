...
export default class WebServer {
    ...
    constructor() {
        ...
        this.app.use(serveStaticMw({
            ...
            roots: [
                txDevEnv.ENABLED
                    ...
                    : path.join(vibeEnv.txaPath, 'panel'),
                path.join(vibeEnv.txaPath, 'web/public'),
            ],
            onReady: () => {
                this.isServing = true;
            },
        ...
        // Multi-Hosting server configuration swapper middleware
        this.app.use(async (ctx, next) => {
            const serverId = ctx.headers['x-vibesm-server-id'] || ctx.query.serverId;
            if (serverId && serverId !== 'primary') {
                try {
                    const { getServerContextStore } = await import('@routes/multiHosting');
                    const store = await getServerContextStore(serverId as string);
                    if (store) {
                        const { vibeConfigStoreContext } = await import('@core/vibeSM');
                        return vibeConfigStoreContext.run(store, next);
                    }
                } catch (e) {
                    console.verbose.error(`Error swapping multi-hosting server config: ${(e as any).message}`);
                }
            }
            return next();
        });
        ...
        try {
            const listenErrorHandler = (error: any) => {
                ...
                fatalError.WebServer(0, [
                    ...
                    'Maybe you already have another vibeSM running in this port.',
                    'If you want to run multiple vibeSM instances, check the documentation for the port convar.',
            ...
            const netInterface = vibeHostConfig.netInterface ?? '0.0.0.0';
            if (vibeHostConfig.netInterface) {
                console.warn(`Starting with interface ${vibeHostConfig.netInterface}.`);
                console.warn('If the HTTP server doesn\'t start, this is probably the reason.');
            }

            this.httpServer.listen(vibeHostConfig.txaPort, netInterface, async () => {
                //Sanity check on globals, to _guarantee_ all routes will have access to them
                if (!vibeCore || isProxy(vibeCore) || !vibeConfig || !vibeManager) {
                    console.dir({
                        vibeCore: Boolean(vibeCore),
                        vibeCoreType: isProxy(vibeCore) ? 'proxy' : 'not proxy',
                        vibeConfig: Boolean(vibeConfig),
                        vibeManager: Boolean(vibeManager),
                    });
