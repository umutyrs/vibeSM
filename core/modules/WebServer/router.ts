...
export default () => {
    ...
    const authLimiter = KoaRateLimit({
        driver: 'memory',
        db: new Map(),
        duration: vibeConfig.webServer.limiterMinutes * 60 * 1000, // 15 minutes
        errorMessage: JSON.stringify({
            //Duplicated to maintain compatibility with all auth api routes
            error: `Too many attempts. Blocked for ${vibeConfig.webServer.limiterMinutes} minutes.`,
            errorTitle: 'Too many attempts.',
            errorMessage: `Blocked for ${vibeConfig.webServer.limiterMinutes} minutes.`,
        }),
        max: vibeConfig.webServer.limiterAttempts,
        disableHeader: true,
    ...
    router.get('/legacy/fileEditor', webAuthMw, routes.fileEditor_page);
    ...
    router.get('/settings/locale', apiAuthMw, routes.settings_getLocale);
    router.post('/settings/locale', apiAuthMw, routes.settings_saveLocale);
    ...
    router.post('/fileEditor/save', apiAuthMw, routes.fileEditor_save);
    ...
    */
    ...
    //Multi-Hosting routes
    router.get('/multi-hosting/servers', apiAuthMw, routes.multiHosting.listServers);
