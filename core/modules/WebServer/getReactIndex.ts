...
import { vibeEnv, txDevEnv, vibeHostConfig } from "@core/globalData";
...
export default async function getReactIndex(ctx: CtxWithVars | AuthedCtx) {
    ...
    if (txDevEnv.ENABLED || !htmlFile) {
        try {
            const indexPath = txDevEnv.ENABLED
                ...
                : path.join(vibeEnv.txaPath, 'panel/index.html')
    ...
    if (typeof queryLang === 'string' && queryLang) {
        ...
        const cookieLang = ctx.cookies.get('vibeSM-language');
        if (cookieLang) {
            lang = cookieLang;
        }
    }
    ...
    const injectedConsts: InjectedTxConsts = {
        //env
        fxsVersion: vibeEnv.fxsVersionTag,
        fxsOutdated: vibeCore.updateChecker.fxsUpdateData,
        vibeVersion: vibeEnv.vibeVersion,
        txaOutdated: vibeCore.updateChecker.txaUpdateData,
        serverTimezone,
        isWindows: vibeEnv.isWindows,
        isWebInterface: ctx.txVars.isWebInterface,
        showAdvanced: (txDevEnv.ENABLED || console.isVerbose),
        hasMasterAccount: vibeCore.adminStore.hasAdmins(true),
        defaultTheme: tmpDefaultTheme,
        customThemes: tmpCustomThemes.map(({ name, isDark }) => ({ name, isDark })),
        adsData: vibeEnv.adsData,
        providerLogo: vibeHostConfig.providerLogo,
        providerName: vibeHostConfig.providerName,
        hostConfigSource: vibeHostConfig.sourceName,
        ...
        //translation
        language: lang,
        phrases: vibeCore.translator.getLanguagePhrases(lang),
        locales,
    };
    ...
    const themeCookie = ctx.cookies.get('vibeSM-theme');
    if (themeCookie) {
