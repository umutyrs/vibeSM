...
export class AuthedAdmin {
    ...
    public logAction(action: string): void {
        vibeCore.logger.admin.write(this.name, action);
    };
    ...
    public logCommand(data: string): void {
        vibeCore.logger.admin.write(this.name, data, 'command');
    };
...
export const checkRequestAuth = (
    ...
    const hasNuiToken = typeof reqHeader['x-vibesm-token'] === 'string' || typeof reqHeader['x-txadmin-token'] === 'string';
    return hasNuiToken
...
export const normalAuthLogic = (
    ...
    try {
        ...
        const vaultAdmin = vibeCore.adminStore.getAdminByName(sessAuth.username);
        if (!vaultAdmin) {
...
export const nuiAuthLogic = (
    ...
    try {
        ...
        if (
            !isLocalRequest
            && !vibeEnv.isZapHosting
            && !vibeConfig.webServer.disableNuiSourceCheck
        ) {
            console.verbose.warn(`NUI Auth Failed: reqIp "${reqIp}" not a local or allowed address.`);
            return failResp('Invalid Request: source');
        }

        // Support both rebranded and legacy headers
        const token = reqHeader['x-vibesm-token'] ?? reqHeader['x-txadmin-token'];
        const rawIdentifiers = reqHeader['x-vibesm-identifiers'] ?? reqHeader['x-txadmin-identifiers'];
        ...
        // Check token value
        if (token !== vibeCore.webServer.luaComToken) {
            const expected = vibeCore.webServer.luaComToken;
            const censoredExpected = expected.slice(0, 6) + '...' + expected.slice(-6);
            console.verbose.warn(`NUI Auth Failed: token received '${token}' !== expected '${censoredExpected}'.`);
        ...
        // Searching for admin in AdminStore
        const vaultAdmin = vibeCore.adminStore.getAdminByIdentifiers(identifiers);
        if (!vaultAdmin) {
            if(!rawIdentifiers.includes('license:')) {
