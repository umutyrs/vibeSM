...
/**
 * Protects vibeSM against a massive HTTP load, no matter the source of the requests.
...
const checkHttpLoad = () => {
    ...
    if (reqCounter >= RPM_CHECK_INTERVAL) {
        ...
        if (requestsPerMinute > HIGH_LOAD_RPM) {
            ...
            console.warn(`vibeSM is under high HTTP load: ~${Math.round(requestsPerMinute / 1000)}k reqs/min.`);
        } else {
    ...
    if (isUnderHighLoad && reqCounter % 100 === 0) {
        ...
        if (heapUsed > HEAP_USED_LIMIT) {
            console.majorMultilineError([
                ...
                'You can join https://discord.gg/vibeSM for support.'
            ]);
            //Resetting counter
            reqCounter = 0;
