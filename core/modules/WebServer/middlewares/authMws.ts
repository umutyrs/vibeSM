...
export const hostAuthMw = async (ctx: InitializedCtx, next: Function) => {
    const docs = 'https://aka.cfx.re/vibesm-env-config';
    ...
    //Token undefined
    if (!vibeHostConfig.hostApiToken) {
        return ctx.send({
            error: 'token not configured',
            desc: 'need to configure the VIBEHOST_API_TOKEN environment variable to be able to use the status endpoint',
            docs,
        });
    }
    ...
    const headerToken = ctx.headers['x-vibesm-envtoken'];
    ...
    if (tokenProvided !== vibeHostConfig.hostApiToken) {
        return ctx.send({
            error: 'invalid token',
            desc: 'the token provided does not match the VIBEHOST_API_TOKEN environment variable',
            docs,
        });
    }
...
export const intercomAuthMw = async (ctx: InitializedCtx, next: Function) => {
    if (
        typeof ctx.request.body?.vibeSMToken !== 'string'
        || ctx.request.body.vibeSMToken !== vibeCore.webServer.luaComToken
    ) {
...
export const apiAuthMw = async (ctx: InitializedCtx, next: Function) => {
    ...
    if (ctx.txVars.isWebInterface) {
        ...
        const headerToken = ctx.headers['x-vibesm-csrftoken'] || ctx.headers['x-txadmin-csrftoken'];
        if (!sessToken || !headerToken || sessToken !== headerToken) {
            ...
            const msg = (headerToken)
                ...
                : 'Error: Missing HTTP header \'x-vibesm-csrftoken\'. This likely means your files are not updated or you are using some reverse proxy that is removing this header from the HTTP request.';
