...
const getWebViewPath = (view: string) => {
    ...
    return path.join(vibeEnv.txaPath, 'web', view + '.ejs');
};
...
function getEjsOptions(filePath: string) {
    const webTemplateRoot = path.resolve(vibeEnv.txaPath, 'web');
    const webCacheDir = path.resolve(vibeEnv.txaPath, 'web-cache', filePath);
    return {
        cache: true,
...
async function renderView(
    ...
    data.isLinux = !vibeEnv.isWindows;
...
export default async function ctxUtilsMw(ctx: CtxWithVars, next: Next) {
    ...
    const renderUtil = async (view: string, data?: { headerTitle?: string, [key: string]: any }) => {
        ...
        const baseViewData = {
            ...
            uiTheme: legacyTheme,
            fxServerVersion: vibeEnv.fxsVersionTag,
            vibeSMVersion: vibeEnv.vibeVersion,
            txAdminVersion: vibeEnv.vibeVersion,
            ...
            jsInjection: getJavascriptConsts({
                ...
                PAGE_TITLE: data?.headerTitle ?? 'vibeSM',
            }),
        };
