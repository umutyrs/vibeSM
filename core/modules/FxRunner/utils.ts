...
export const getMutableConvars = (isCmdLine = false) => {
    const checkPlayerJoin = vibeConfig.banlist.enabled || (
        vibeConfig.whitelist.mode !== 'disabled'
        && vibeConfig.whitelist.mode !== 'external'
    );
    const appearAllowlisted = vibeConfig.whitelist.mode !== 'disabled';
    const convars: RawConvarSetTuple[] = [
        ['setr', 'locale', vibeConfig.general.language ?? 'en'],
        ['set', 'serverName', vibeConfig.general.serverName ?? 'vibeSM'],
        ['set', 'checkPlayerJoin', checkPlayerJoin],
        ['set', 'menuAlignRight', vibeConfig.gameFeatures.menuAlignRight],
        ['set', 'menuPageKey', vibeConfig.gameFeatures.menuPageKey],
        ['set', 'playerModePtfx', vibeConfig.gameFeatures.playerModePtfx],
        ['set', 'hideAdminInPunishments', vibeConfig.gameFeatures.hideAdminInPunishments],
        ['set', 'hideAdminInMessages', vibeConfig.gameFeatures.hideAdminInMessages],
        ['set', 'hideDefaultAnnouncement', vibeConfig.gameFeatures.hideDefaultAnnouncement],
        ['set', 'hideDefaultDirectMessage', vibeConfig.gameFeatures.hideDefaultDirectMessage],
        ['set', 'hideDefaultWarning', vibeConfig.gameFeatures.hideDefaultWarning],
        ['set', 'hideDefaultScheduledRestartWarning', vibeConfig.gameFeatures.hideDefaultScheduledRestartWarning],
...
const polishConvarSetTuple = ([setter, name, value]: RawConvarSetTuple, isCmdLine = false): ConvarSetTuple => {
    return [
        ...
        name.startsWith('sv_') ? name : 'vibeSM-' + name,
        value.toString(),
    ];
}
 ...
 */
const vibeCoreEndpoint = vibeHostConfig.netInterface
    ? `${vibeHostConfig.netInterface}:${vibeHostConfig.txaPort}`
    : `127.0.0.1:${vibeHostConfig.txaPort}`;
let osSpawnVars: OsSpawnVars;
if (vibeEnv.isWindows) {
    osSpawnVars = {
        bin: `${vibeEnv.fxsPath}/FXServer.exe`,
        args: [],
    };
} else {
    const alpinePath = path.resolve(vibeEnv.fxsPath, '../../');
    osSpawnVars = {
...
export const getFxSpawnVariables = (): FxSpawnVariables => {
    if (!vibeConfig.server.dataPath) throw new Error('Missing server data path');

    const cmdArgs = [
        ...osSpawnVars.args,
        getMutableConvars(true), //those are the ones that can change without restart
        vibeConfig.server.startupArgs,
        '+set', 'onesync', vibeConfig.server.onesync,
        '+sets', 'vibeSM-version', vibeEnv.vibeVersion,
        '+setr', 'vibeSM-menuEnabled', vibeConfig.gameFeatures.menuEnabled,

        // Convars expected by the FiveM FXServer binary (hardcoded in C++)
        '+set', 'txAdminServerMode', 'true',
        '+set', 'txAdmin-luaComHost', vibeCoreEndpoint,
        '+set', 'txAdmin-luaComToken', vibeCore.webServer.luaComToken,

        // Convars expected by the vibeSM resource/script files
        '+set', 'vibeSM-luaComHost', vibeCoreEndpoint,
        '+set', 'vibeSM-luaComToken', vibeCore.webServer.luaComToken,
        '+set', 'vibeSMServerMode', 'true', //Can't change this one due to fxserver code compatibility

        '+exec', vibeConfig.server.cfgPath,
        '+ensure', 'monitor',
    ].flat(2).map(String);
    ...
    return {
        ...
        dataPath: vibeConfig.server.dataPath,
        cfgPath: vibeConfig.server.cfgPath,
    }
}
...
export const setupCustomLocaleFile = async () => {
    if (vibeConfig.general.language !== 'custom') return;
    const srcPath = vibeCore.translator.customLocalePath;
    const destRuntimePath = path.resolve(vibeEnv.txaPath, '.runtime');
