...
export default class FxRunner {
    ...
    /**
     * Gracefully shutdown when vibeSM gets an exit event.  
    ...
    public signalStartReady() {
        if (!vibeConfig.server.autoStart) return;

        if (!this.isConfigured) {
            return console.warn('Please open vibeSM on the browser to configure your server.');
        }
        ...
        if (vibeConfig.server.quiet || vibeHostConfig.forceQuietMode) {
     ...
     * NOTE: Don't use vibeConfig in here to avoid race conditions.
     */
    public async spawnServer(shouldAnnounce = false) {
        const { vibeConfigStoreContext } = require('@core/vibeSM');
        return vibeConfigStoreContext.exit(() => this.spawnServerInternal(shouldAnnounce));
    }

    private async spawnServerInternal(shouldAnnounce = false) {
        //If vibeSM is shutting down
        if (vibeManager.isShuttingDown) {
            const msg = `Cannot start the server while vibeSM is shutting down.`;
            console.error(msg);
            return msg;
        }
        ...
        // Check if dataPath is in use by a multi-hosted server
        try {
            const { runningServers, readServers, normalizePath } = require('../../routes/multiHosting');
            if (runningServers && readServers && normalizePath) {
                const normPrimary = normalizePath(vibeConfig.server.dataPath);
                const servers = await readServers();
                for (const srv of servers) {
                    if (runningServers.has(srv.id)) {
                        const normSrv = normalizePath(srv.dataPath);
                        if (normPrimary === normSrv) {
                            const msg = `Cannot start primary server because its data folder is already in use by multi-hosted server "${srv.name}".`;
                            console.error(msg);
                            return msg;
                        }
                    }
        ...
        try {
            ...
            if ((error as any).message.includes('unreadable')) {
                ...
                console.error('To fix this issue, open the vibeSM web interface then go to "Settings > FXServer" and fix the "Server Data Folder" and "CFG File Path".');
            }
            return errMsg;
        }

        //Reseting monitor stats
        vibeCore.fxMonitor.resetState();

        //Resetting frontend playerlist
        vibeCore.webServer.webSocket.buffer('playerlist', {
        ...
        childProc.stdout.on('data',
            vibeCore.logger.fxserver.writeFxsOutput.bind(
                vibeCore.logger.fxserver,
    ...
    public async restartServer(reason: string, author: string | typeof SYM_SYSTEM_AUTHOR) {
        const { vibeConfigStoreContext } = require('@core/vibeSM');
        return vibeConfigStoreContext.exit(() => this.restartServerInternal(reason, author));
    }

    private async restartServerInternal(reason: string, author: string | typeof SYM_SYSTEM_AUTHOR) {
        ...
        try {
            //Restart server
            const killError = await this.killServerInternal(reason, author, true);
            ...
            //Start server again :)
            return await this.spawnServerInternal();
        } catch (error) {
    ...
    public async killServer(reason: string, author: string | typeof SYM_SYSTEM_AUTHOR, isRestarting = false) {
        const { vibeConfigStoreContext } = require('@core/vibeSM');
        return vibeConfigStoreContext.exit(() => this.killServerInternal(reason, author, isRestarting));
    }

    private async killServerInternal(reason: string, author: string | typeof SYM_SYSTEM_AUTHOR, isRestarting = false) {
        if (!this.proc) return null; //nothing to kill

        //Prepare vars
        const shutdownDelay = Math.max(vibeConfig.server.shutdownNoticeDelayMs, MIN_KILL_DELAY);
        ...
        try {
            ...
            if (this.proc.isAlive) {
                this.sendEvent('serverShuttingDown', {
                    delay: vibeConfig.server.shutdownNoticeDelayMs,
                    author: typeof author === 'string' ? author : 'vibeSM',
                    message: vibeCore.translator.t(`server_actions.${messageType}`, tOptions),
                });
            ...
            vibeCore.fxResources.handleServerClose();
            vibeCore.fxPlayerlist.handleServerClose(debugInfo.mutex);
            vibeCore.metrics.svRuntime.logServerClose(reasonString);
            vibeCore.discordBot.sendAnnouncement({
                type: messageColor,
                description: {
     ...
     * Useful for when we change vibeSM settings and want it to reflect on the server.
     * This will also fire the `vibeSM:event:configChanged`
     */
    ...
    /**
     * Fires an `vibeSM:event` inside the server via srvCmd > stdin > command > lua broadcaster.  
    ...
    public sendRawCommand(command: string, author: string | typeof SYM_SYSTEM_AUTHOR) {
        ...
        try {
            ...
            if (author === SYM_SYSTEM_AUTHOR) {
                vibeCore.logger.fxserver.logSystemCommand(command);
            } else {
                vibeCore.logger.fxserver.logAdminCommand(author, command);
            }
            return success;
        } catch (error) {
    ...
    public get isConfigured() {
        return typeof vibeConfig.server.dataPath === 'string'
            && vibeConfig.server.dataPath.length > 0
            && typeof vibeConfig.server.cfgPath === 'string'
            && vibeConfig.server.cfgPath.length > 0;
    }
    ...
    public get serverPaths() {
        ...
        return {
            dataPath: path.normalize(vibeConfig.server.dataPath!), //to maintain consistency
            cfgPath: resolveCFGFilePath(vibeConfig.server.cfgPath, vibeConfig.server.dataPath!),
        }
        // return {
        //     data: {
        //         absolute: 'xxx',
    ...
    public get restartSpawnDelay() {
        let ms = vibeConfig.server.restartSpawnDelayMs;
        let isBackoff = false;
