...
export default class FxScheduler {
    ...
    setNextSkip(enabled: boolean, author?: string) {
        if (enabled) {
            ...
            if (prevMinuteFloorTs) {
                //Dispatch `vibeSM:events:scheduledRestartSkipped`
                vibeCore.fxRunner.sendEvent('scheduledRestartSkipped', {
                ...
                //FIXME: deprecate
                vibeCore.fxRunner.sendEvent('skippedNextScheduledRestart', {
                    secondsRemaining: Math.floor((prevMinuteFloorTs - Date.now()) / 1000),
                    temporary
                });
            }
    ...
    setNextTempSchedule(timeString: string) {
        ...
        //Check validity
        if (Array.isArray(vibeConfig.restarter.schedule) && vibeConfig.restarter.schedule.length) {
            const { valid } = parseSchedule(vibeConfig.restarter.schedule);
    ...
    async checkSchedule(calculateOnly = false) {
        ...
        } else if (Array.isArray(vibeConfig.restarter.schedule) && vibeConfig.restarter.schedule.length) {
            const { valid } = parseSchedule(vibeConfig.restarter.schedule);
        ...
        if (nextDistMins === 0) {
            ...
            this.triggerServerRestart(
                ...
                vibeCore.translator.t('restarter.schedule_reason', { time: nextRestart.string }),
            );
            ...
            //Dispatch `vibeSM:events:scheduledRestart` 
            vibeCore.fxRunner.sendEvent('scheduledRestart', {
                secondsRemaining: nextDistMins * 60,
                translatedMessage: vibeCore.translator.t('restarter.schedule_warn', tOptions)
            });
        }
    }
    ...
    async triggerServerRestart(reasonInternal: string, reasonTranslated: string) {
        //Sanity check
        if (vibeCore.fxRunner.isIdle || !vibeCore.fxRunner.child?.isAlive) {
        ...
        vibeCore.logger.admin.write('SCHEDULER', logMessage);
        vibeCore.logger.fxserver.logInformational(logMessage); //just for better visibility
        vibeCore.fxRunner.restartServer(reasonTranslated, SYM_SYSTEM_AUTHOR);
    }
};
