...
export default class FxMonitor {
    ...
    private hasNucleusConnected = false;
    ...
    private setCurrentStatus(newStatus: FxMonitorHealth) {
        ...
        vibeCore.discordBot.updateBotStatus().catch((e) => { });
        vibeCore.webServer.webSocket.pushRefresh('status');
    }


    /**
     * Marks the server as online and runs the one-shot boot completion tasks.
     */
    private setServerHealthy(processUptime: number) {
    ...
    public resetState() {
        ...
        this.hasNucleusConnected = false;
    ...
    public handleNucleusConnected() {
        this.hasNucleusConnected = true;
        this.updateStatus();
    }
    ...
    private updateStatus() {
        ...
        if (result.action === 'RESTART') {
            if (vibeCore.fxRunner.isIdle) return; //should never happen

            //Logging
            const timesSuffix = result.times ? ` ${result.times}.` : '.';
            const logMessage = `Restarting server: ${result.reason}` + timesSuffix;
            console.error(logMessage);
            vibeCore.logger.admin.writeSystem('MONITOR', logMessage);
            vibeCore.logger.fxserver.logInformational(logMessage); //just for better visibility
            for (const line of styledIssues) {
                vibeCore.logger.fxserver.logInformational(line);
                console.error(line);
            }
            ...
            vibeCore.fxRunner.restartServer(
                vibeCore.translator.t('restarter.server_unhealthy_kick_reason'),
                SYM_SYSTEM_AUTHOR,
            );
            vibeCore.metrics.txRuntime.registerFxserverRestart(result.cause);
        }
    }
     ...
     *  - HeartBeat: receiving an intercom POST or FD3 vibeSMHeartBeat event
     */
    private calculateMonitorStatus(): ProcessStatusResult {
        ...
        if (this.swLastStatusUpdate.isOver(10)) {
            console.error(`vibeSM was frozen for ${this.swLastStatusUpdate.elapsed - 1} seconds for unknown reason (random issue, VPS Lag, DDoS, etc).`);
            this.swLastStatusUpdate.restart();
            return {
                action: 'SKIP',
                reason: 'vibeSM was frozen',
            }
        }
        this.swLastStatusUpdate.restart();
        ...
        const processUptime = Math.floor((vibeCore.fxRunner.child?.uptime ?? 0) / 1000);
        const timeTags = getMonitorTimeTags(heartBeat, healthCheck, processUptime)
        // console.dir({
        //     timeTag: timeTags.withProc,
        //     heartBeat,
        //     healthCheck,
        //     child: vibeCore.fxRunner.child?.status,
        // }, { compact: true }); //DEBUG
        ...
        if (vibeCore.fxRunner.child?.status === ChildProcessState.Destroyed) {
        ...
        if (heartBeat.state === MonitorState.HEALTHY && healthCheck.state === MonitorState.HEALTHY) {
            this.setServerHealthy(processUptime);
            return {
                action: 'SKIP',
        ...
        // Some FXServer/runtime combinations expose /dynamic.json before or without
        // the internal monitor resource heartbeat channel. A healthy HTTP endpoint
        // proves the server booted, so do not kill it for a missing first heartbeat.
        if (heartBeat.state === MonitorState.PENDING && healthCheck.state === MonitorState.HEALTHY) {
            this.setServerHealthy(processUptime);
            return {
                action: 'SKIP',
        ...
        if (processUptime < vibeConfig.restarter.bootGracePeriod) {
            return {
                action: 'SKIP',
                reason: `FXServer is ${currentStatusString}, still in grace period of ${vibeConfig.restarter.bootGracePeriod}s.`,
            };
        }
        ...
        if (heartBeat.state !== MonitorState.HEALTHY) {
            ...
            if (heartBeat.state === MonitorState.PENDING) {
                ...
                const resBoot = vibeCore.fxResources.bootStatus;
                if (resBoot.current?.time.isOver(vibeConfig.restarter.resourceStartingTolerance)) {
                    //Resource boot timed out
                    return {
                        action: 'RESTART',
                        times: timeTags.withProc,
                        reason: `Resource "${resBoot.current.name}" failed to start within the ${secs(vibeConfig.restarter.resourceStartingTolerance)} time limit`,
                        cause: 'bootTimeout',
                    }
        ...
        if (
            ...
            vibeCore.discordBot.sendAnnouncement({
                ...
                description: {
                    ...
                    data: { servername: vibeConfig.general.serverName },
                },
            });

            // Dispatch `vibeSM:events:announcement`
            vibeCore.fxRunner.sendEvent('announcement', {
                author: 'vibeSM',
                message: vibeCore.translator.t('restarter.partial_hang_warn'),
            });
        }
    ...
    private async performHealthCheck() {
        //Check if the server is supposed to be offline
        const childState = vibeCore.fxRunner.child;
        ...
        if (dynamicJson.sv_maxclients) {
            ...
            vibeCore.cacheStore.set('fxsRuntime:maxClients', maxClients);
            if (vibeHostConfig.forceMaxClients && maxClients > vibeHostConfig.forceMaxClients) {
                vibeCore.fxRunner.sendCommand(
                    'sv_maxclients',
                    [vibeHostConfig.forceMaxClients],
                    SYM_SYSTEM_AUTHOR
                );
                console.error(`${vibeHostConfig.sourceName}: Detected that the server has sv_maxclients above the limit (${vibeHostConfig.forceMaxClients}). Changing back to the limit.`);
                vibeCore.logger.admin.writeSystem('SYSTEM', `changing sv_maxclients back to ${vibeHostConfig.forceMaxClients}`);
            }
        }
    }
    ...
    public handleHeartBeat(source: 'fd3' | 'http') {
        ...
        if (source === 'fd3') {
            if (
                ...
                vibeCore.metrics.txRuntime.registerFxserverHealthIssue('http');
            }
            ...
            if (
                ...
                vibeCore.metrics.txRuntime.registerFxserverHealthIssue('fd3');
            }
    ...
    private async handleBootCompleted(bootDuration: number) {
        ...
        vibeCore.metrics.txRuntime.registerFxserverBoot(bootDuration);
        vibeCore.metrics.svRuntime.logServerBoot(bootDuration);
        vibeCore.fxRunner.signalSpawnBackoffRequired(false);
        ...
        if (infoJson.icon) {
            try {
                ...
                vibeCore.cacheStore.set(iconCacheKey, iconFilename);
            } catch (error) {
                console.error(`Failed to save server icon: ${(error as any).message ?? 'Unknown error'}`);
            }
        } else {
            vibeCore.cacheStore.delete(iconCacheKey);
        }

        //Upserts the runtime data
        vibeCore.cacheStore.upsert('fxsRuntime:bannerConnecting', infoJson.bannerConnecting);
        vibeCore.cacheStore.upsert('fxsRuntime:bannerDetail', infoJson.bannerDetail);
        vibeCore.cacheStore.upsert('fxsRuntime:locale', infoJson.locale);
        vibeCore.cacheStore.upsert('fxsRuntime:projectDesc', infoJson.projectDesc);
        if (infoJson.projectName) {
            vibeCore.cacheStore.set('fxsRuntime:projectName', cleanPlayerName(infoJson.projectName).displayName);
        } else {
            vibeCore.cacheStore.delete('fxsRuntime:projectName');
        }
        vibeCore.cacheStore.upsert('fxsRuntime:tags', infoJson.tags);
    }
    ...
    public get status() {
        ...
        let health = this.currentStatus;
        if (vibeCore.fxRunner.child?.isAlive && !this.tsServerBooted) {
            health = FxMonitorHealth.STARTING;
            healthReason = 'Server is starting';
        }
