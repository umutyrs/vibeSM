...
export default class FxPlayerlist {
    #playerlists = new Map<string, (ServerPlayer | undefined)[]>();
    ...
    #getPlayerlist(mutex?: string): (ServerPlayer | undefined)[] {
        const activeMutex = mutex || vibeCore.fxRunner.child?.mutex || 'primary';
        if (!this.#playerlists.has(activeMutex)) {
            this.#playerlists.set(activeMutex, []);
        }
        return this.#playerlists.get(activeMutex)!;
    }
    ...
    handleServerClose(oldMutex: string) {
        const plist = this.#getPlayerlist(oldMutex);
        ...
        this.#playerlists.delete(oldMutex);
        this.joinLeaveLog = [];
    ...
    handleDbDataSync(dbData: DatabasePlayerType, srcUniqueId: Symbol) {
        for (const plist of this.#playerlists.values()) {
            for (const player of plist) {
                if (
     ...
     */
    getPlayerList(mutex?: string) {
        return this.#getPlayerlist(mutex)
     ...
     */
    getPlayerById(netid: number, mutex?: string) {
        return this.#getPlayerlist(mutex)[netid];
    }
     ...
     * NOTE: this returns the actual object and not a deep clone!
     */
    getOnlinePlayersByLicense(searchLicense: string, mutex?: string) {
        return this.#getPlayerlist(mutex).filter(p => p && p.license === searchLicense && p.isConnected) as ServerPlayer[];
    }

    /**
     * Returns a set of all online players' licenses.
     */
    getOnlinePlayersLicenses(mutex?: string) {
        return new Set(this.#getPlayerlist(mutex).filter(p => p && p.isConnected).map(p => p!.license));
    }
    ...
    dispatchInitialPlayerData(playerId: number, pendingWarn: DatabaseActionWarnType) {
        ...
        vibeCore.fxRunner.sendCommand('txaInitialData', [cmdData], SYM_SYSTEM_AUTHOR);
    }


    /**
     * Handler for all vibeSMPlayerlistEvent structured trace events
     * TODO: use zod for type safety
     */
    async handleServerEvents(payload: any, mutex: string) {
        const currTs = Date.now();
        const plist = this.#getPlayerlist(mutex);
        const serverId = (globalThis as any).multiHostingMutexes?.get(mutex) || 'primary';

        if (payload.event === 'playerJoining') {
            try {
                if (typeof payload.id !== 'number') throw new Error(`invalid player id`);
                if (plist[payload.id] !== undefined) throw new Error(`duplicated player id`);
                const svPlayer = new ServerPlayer(payload.id, payload.player, this);
                plist[payload.id] = svPlayer;
                this.joinLeaveLog.push([currTs, true]);
                vibeCore.logger.server.write([{
                    type: 'playerJoining',
                    src: payload.id,
                    ts: currTs,
                    data: { ids: plist[payload.id]!.ids }
                }], mutex);
                vibeCore.webServer.webSocket.buffer<any>('playerlist', {
                    mutex,
                    serverId,
                    type: 'playerJoining',
