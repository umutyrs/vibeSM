...
//     cnt++;
//     if (cnt > 84) cnt = 1;
//     const mtx = vibeCore.fxRunner.child?.mutex ?? 'UNKNW';
//     const payload = [
//         {
//             src: 'tx',
//             ts: Date.now(),
...
//         },
//     ];
//     vibeCore.logger.server.write(mtx, payload);
// }, 750);


export default class ServerLogger extends LoggerBase {
    constructor(basePath, lrProfileConfig, serverId) {
    ...
    write(data, mutex) {
        ...
        mutex ??= vibeCore.fxRunner.child.mutex ?? 'UNKNW';
        ...
        for (let i = 0; i < data.length; i++) {
            try {
                ...
                //Send to websocket
                const roomName = this.serverId && this.serverId !== 'primary' ? `serverlog#${this.serverId}` : 'serverlog';
                vibeCore.webServer.webSocket.buffer(roomName, eventObject);

                //Write to file
                this.lrStream.write(`${eventString}\n`);
            } catch (error) {
                console.verbose.error('Error processing FD3 vibeSMLogData:');
    ...
    processEvent(eventData, mutex) {
        ...
        if (eventData.src === 'tx') {
            srcObject = { id: false, name: 'vibeSM' };
            srcString = 'vibeSM';

        } else if (typeof eventData.src === 'number' && eventData.src > 0) {
            const player = vibeCore.fxPlayerlist.getPlayerById(eventData.src);
            if (player) {
        ...
        if (eventData.type === 'playerJoining') {
            ...
            if (typeof eventData.data.killer === 'number' && eventData.data.killer > 0) {
                const killer = vibeCore.fxPlayerlist.getPlayerById(eventData.data.killer);
                if (killer) {
            ...
            if (typeof eventData.data?.gameName === 'string' && eventData.data.gameName.length) {
                if(eventData.data.gameName === 'gta5'){
                    ...
                    vibeCore.cacheStore.set('fxsRuntime:gameName', 'redm');
                } else {
                    vibeCore.cacheStore.delete('fxsRuntime:gameName');
                }
            }
            ...
            vibeCore.metrics.txRuntime.menuCommands.count(eventData.data?.action ?? 'unknown');
