...
import { vibeHostConfig } from '@core/globalData.js';
const console = consoleFactory(modulename);


//This regex was done in the first place to prevent fxserver output to be interpreted as vibeSM output by the host terminal
//IIRC the issue was that one user with a TM on their nick was making vibeSM's console to close or freeze. I couldn't reproduce the issue.
...
export default class FXServerLogger extends LoggerBase {
    ...
    constructor(basePath: string, lrProfileConfig: RfsOptions | false, serverId?: string) {
    ...
    private ingest(type: ConsoleLineEnum, data: string, context?: string) {
        ...
        if (!vibeConfig.server.quiet && !vibeHostConfig.forceQuietMode) {
            if (!this.serverId || this.serverId === 'primary') {
                processStdioWriteRaw(stdoutBuffer);
            }
        }

        //For the live console
        const roomName = this.serverId && this.serverId !== 'primary' ? `liveconsole#${this.serverId}` : 'liveconsole';
        vibeCore.webServer.webSocket.buffer(roomName, webBuffer);
    ...
    public logSystemCommand(cmd: string) {
        ...
        //     cmd = chalk.italic(`<broadcasting vibeSM:events:${event}>`);
        // }
