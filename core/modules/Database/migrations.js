...
export default async (dbo) => {
    ...
    if (dbo.data.version > DATABASE_VERSION) {
        fatalError.Database(51, [
            `Your players database is on v${dbo.data.version}, and this vibeSM supports up to v${DATABASE_VERSION}.`,
    ...
    if (dbo.data.version === 2) {
        ...
        console.warn('\t- allow vibeSM to save old player identifiers');
    ...
    if (dbo.data.version !== DATABASE_VERSION) {
        fatalError.Database(52, [
            ...
            'Please make sure your vibeSM is on the most updated version!',
        ]);
    }
