...
export class DbInstance {
    ...
    constructor(customDbPath?: string) {
        this.dbPath = customDbPath || `${vibeEnv.profilePath}/data/playersDB.json`;
    ...
    async setupDatabase() {
        ...
        try {
            ...
            const errTitle = 'Your vibeSM player/actions database could not be loaded.';
            try {
        ...
        try {
            ...
            if (dbo.data.version !== DATABASE_VERSION) {
                await this.backupDatabase(`${vibeEnv.profilePath}/data/playersDB.backup.v${dbo.data.version}.json`);
            ...
            if (
                ...
                fatalError.Database(2, [
                    'Your vibeSM player/actions database is corrupted!',
    ...
    private async checkWriteNeeded() {
        ...
        if (this.#writePending === SavePriority.HIGH || sinceLastWrite > SAVE_CONFIG[this.#writePending].interval) {
            ...
            vibeCore.metrics.txRuntime.databaseSaveTime.count(timeElapsed.milliseconds);
        }
    }
