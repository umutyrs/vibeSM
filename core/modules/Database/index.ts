...
export default class Database {
    ...
    constructor() {
        const primaryDb = new DbInstance();
        this.#db = new Proxy(primaryDb, {
            get(target, prop, receiver) {
                try {
                    const { vibeConfigStoreContext } = require('@core/vibeSM');
                    const store = vibeConfigStoreContext.getStore();
                    if (store && store.db) {
                        const val = Reflect.get(store.db, prop);
                        return typeof val === 'function' ? val.bind(store.db) : val;
                    }
                } catch (e) {}
        ...
        const optimizerTask = () => {
            if(vibeManager.configState === TxConfigState.Ready) {
