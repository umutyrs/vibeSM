...
export default class CleanupDao {
    ...
    runDailyOptimizer() {
        ...
        try {
            ...
            wlRequestsRemoved = vibeCore.database.whitelist.removeManyRequests(wlRequestsFilter).length;

            const wlApprovalsFilter = (req: DatabaseWhitelistApprovalsType) => {
                return (req.tsApproved < sevenDaysAgo);
            }
            wlApprovalsRemoved = vibeCore.database.whitelist.removeManyApprovals(wlApprovalsFilter).length;
        } catch (error) {
            const msg = `Failed to optimize players database with error: ${(error as Error).message}`;
            console.error(msg);
        }
        ...
        console.ok(`Internal Database optimized. This applies only for the vibeSM internal database, and does not affect your MySQL or framework (ESX/QBCore/etc) databases.`);
