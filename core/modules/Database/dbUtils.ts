...
const noIdErrorMessage = 'Unnable to generate new Random ID possibly due to the decreased available entropy. Please send a screenshot of the detailed information in the terminal for the vibeSM devs.';
...
const printDiagnostics = async () => {
    ...
    console.error(`vibeSM: ${vibeEnv.vibeVersion}`);
    console.error(`FXServer: ${vibeEnv.fxsVersionTag}`);
    console.error(`Provider: ${vibeHostConfig.providerName ?? 'none'}`);
