...
const footer = {
    ...
    text: `vibeSM ${vibeEnv.vibeVersion}`,
}
...
export default async (interaction: CommandInteraction) => {
    ...
    if (adminInfoFlag) {
        const admin = vibeCore.adminStore.getAdminByProviderUID(interaction.user.id);
        if (!admin) {
            return await interaction.reply(embedder.danger('You cannot use the `admininfo` option if you are not a vibeSM admin.'));
        } else {
    ...
    if (!players.length) {
        return await interaction.reply(embedder.warning(`Identifier (\`${searchId}\`) does not seem to be associated to any player in the vibeSM Database.`));
    } else if (players.length > 10) {
        return await interaction.reply(embedder.warning(`The identifier (\`${searchId}\`) is associated with more than 10 players, please use the vibeSM Web Panel to search for it.`));
    }
