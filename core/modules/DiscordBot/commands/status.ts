...
If you just tried to edit a placeholder like \`{{serverBrowserUrl}}\` or \`{{serverJoinUrl}}\`, remember that those placeholders are replaced automatically by vibeSM, meaning you do not need to edit them at all.`
...
export const generateStatusMessage = (
    rawEmbedJson: string = vibeConfig.discordBot.embedJson,
    rawEmbedConfigJson: string = vibeConfig.discordBot.embedConfigJson
) => {
    ...
    const serverCfxId = vibeCore.cacheStore.get('fxsRuntime:cfxId');
    ...
    const placeholders = {
        ...
        serverMaxClients: vibeCore.cacheStore.get('fxsRuntime:maxClients') ?? 'unknown',
        serverClients: vibeCore.fxPlayerlist.onlineCount,
    ...
    const schedule = vibeCore.fxScheduler.getStatus();
    ...
    try {
        ...
        embed.setFooter({
            ...
            text: `vibeSM ${vibeEnv.vibeVersion} • Updated every minute`,
...
export const removeOldEmbed = async (interaction: ChatInputCommandInteraction) => {
    const oldChannelId = vibeCore.cacheStore.get('discord:status:channelId');
    const oldMessageId = vibeCore.cacheStore.get('discord:status:messageId');
...
export default async (interaction: ChatInputCommandInteraction) => {
    ...
    try {
        ...
        vibeCore.cacheStore.delete('discord:status:channelId');
        vibeCore.cacheStore.delete('discord:status:messageId');
        if (isRemoveOnly) {
    ...
    try {
        ...
        vibeCore.cacheStore.set('discord:status:channelId', interaction.channelId);
        vibeCore.cacheStore.set('discord:status:messageId', newMessage.id);
    } catch (error) {
        let msg: string;
