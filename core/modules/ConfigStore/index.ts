...
export default class ConfigStore /*does not extend TxModuleBase*/ {
    ...
    constructor(customConfigFilePath?: string) {
        if (customConfigFilePath) {
            this.configFilePath = customConfigFilePath;
            this.changelogFilePath = path.join(path.dirname(customConfigFilePath), 'data', 'configChangelog.json');
        } else {
            this.configFilePath = `${vibeEnv.profilePath}/config.json`;
            this.changelogFilePath = `${vibeEnv.profilePath}/data/configChangelog.json`;
        }
        //Load raw file
    ...
    */
    ...
    public saveConfigs(inputConfig: PartialTxConfigsToSave, author: string | null) {
        ...
        this.logChanges(author ?? 'vibeSM', processed.storedKeysChanges.list);
    ...
    private logChanges(author: string, keysUpdated: string[]) {
        vibeCore.logger.admin.write(author, `Config changes: ${keysUpdated.join(', ')}`);
        this.changelog.push({
            author,
            ts: Date.now(),
            keys: keysUpdated,
