...
const saveBackupFile = (version: number) => {
    ...
    fs.copyFileSync(
        `${vibeEnv.profilePath}/config.json`,
        `${vibeEnv.profilePath}/${bkpFileName}`,
    );
...
export const migrateConfigFile = (fileData: any): ConfigFileData => {
    ...
    if ('version' in fileData && typeof fileData.version !== 'number') {
        fatalError.ConfigStore(20, 'Your vibeSM config.json version is not a number!');
    }
    if (typeof fileData.version === 'number' && fileData.version > CONFIG_VERSION) {
        fatalError.ConfigStore(21, [
            `Your config.json file is on v${fileData.version}, and this vibeSM supports up to v${CONFIG_VERSION}.`,
            'This means you likely downgraded your vibeSM or FXServer.',
            'Please make sure your vibeSM is updated!',
            '',
            'If you want to downgrade FXServer (the "artifact") but keep vibeSM updated,',
            ...
            ['File Path', `${vibeEnv.profilePath}/config.json`],
        ]);
    }
    ...
    if (!('version' in fileData) && 'global' in fileData && 'fxRunner' in fileData) {
        console.warn('Updating your vibeSM config.json from v1 to v2.');
        oldVersion ??= 1;
