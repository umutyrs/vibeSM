...
export default class Translator {
    ...
    public readonly customLocalePath = vibeHostConfig.dataSubPath('locale.json');
    ...
    setupTranslator(isFirstTime = false) {
        try {
            this.canonical = Intl.getCanonicalLocales(vibeConfig.general.language.replace(/_/g, '-'))[0];
        } catch (error) {
            this.canonical = 'en-GB';
        }

        try {
            const phrases = this.getLanguagePhrases(vibeConfig.general.language);
