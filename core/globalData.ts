...
//Simple env vars
const ignoreDeprecatedConfigs = process.env?.VIBEHOST_IGNORE_DEPRECATED_CONFIGS === 'true';
...
const handleMultiVar = <T extends ZodSchema>(
    ...
    const whichAlt = zapcfg !== undefined ? 'vibeSMZapConfig.json' : 'ConVar';
    if (procenv !== undefined) {
        console.warn(`WARNING: Both the environment variable 'VIBEHOST_${name}' and the ${whichAlt} equivalent are set. The environment variable will be prioritized.`);
        return procenv;
    }
    const parsed = schema.safeParse(alt);
    if (!parsed.success) {
        fatalError.GlobalData(20, [
            `Invalid value for the VIBEHOST_${name}-equivalent config in ${whichAlt}.`,
            ['Value', alt],
            'For more information: https://aka.cfx.re/vibesm-env-config',
...
if (devVars.ENABLED) {
    console.debug('Starting vibeSM in DEV mode.');
    if (!devVars.SRC_PATH || !devVars.VITE_URL) {
        fatalError.GlobalData(8, 'Missing VIBEDEV_VITE_URL and/or VIBEDEV_SRC_PATH env variables.');
    }
...
if (!fxsVerParsed.valid) {
    ...
    console.error('And because of that, there is no guarantee that vibeSM will work properly.');
    ...
    fatalError.GlobalData(2, [
        'This version of FXServer is too outdated and NOT compatible with vibeSM',
...
//Getting vibeSM version
if (!nativeVars.txaResourceVersion) {
    fatalError.GlobalData(3, [
        'vibeSM version not set or in the wrong format.',
        ['Detected version', nativeVars.txaResourceVersion],
    ]);
}
const vibeVersion = nativeVars.txaResourceVersion;

//Get vibeSM Resource Path
if (!nativeVars.txaResourcePath) {
    fatalError.GlobalData(4, [
        'Could not resolve vibeSM resource path.',
...
let profileVar = nativeVars.vibeSMProfile;
if (profileVar) {
...
if (!ignoreDeprecatedConfigs) {
    ...
    const zapCfgFilePath = path.join(dataPath, 'vibeSMZapConfig.json');
    try {
...
const txaUrl = hostVars.VIBE_URL;

//vibeSM port
const txaPort = handleMultiVar(
    'VIBE_PORT',
    hostEnvVarSchemas.VIBE_PORT,
    hostVars.VIBE_PORT,
    zapVars?.vibeSMPort,
    nativeVars.vibeSMPort,
) ?? 40120;
 ...
 */
if (ignoreDeprecatedConfigs) {
    console.verbose.debug('VIBEHOST_IGNORE_DEPRECATED_CONFIGS is set to true. Ignoring deprecated configs.');
}

const isPterodactyl = !isWindows && process.env?.VIBESM_ENABLE === '1';
const isZapHosting = providerName === 'ZAP-Hosting';

//Quick config to disable ads
const displayAds = process.env?.VIBEHOST_TMP_HIDE_ADS !== 'true' || isPterodactyl || isZapHosting;
...
export const vibeEnv = Object.freeze({
    //Calculated
    isWindows,
...
export const vibeHostConfig = Object.freeze({
    //General
    dataPath,
...
//DEBUG
// console.dir(vibeEnv, { compact: true });
// console.dir(txDevEnv, { compact: true });
// console.dir(vibeHostConfig, { compact: true });
