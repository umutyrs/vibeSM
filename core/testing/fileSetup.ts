...
vi.stubGlobal('GetConvar', (varName: string, defaultValue: string) => {
    if (varName === 'version') {
        if (os.platform() === 'win32') {
            return 'FXServer-test/vibesm SERVER v1.0.0.55555 win32';
        } else {
            return 'FXServer-test/vibesm v1.0.0.55555 linux';
        }
    } else if (varName === 'citizen_root') {
        return inject('fxsPath');
    } else if (varName === 'vibeSMDevMode') {
        return 'false';
    } else if (varName === 'vibeSMVerbose') {
        return 'false';
    } else {
        return defaultValue;
    }
