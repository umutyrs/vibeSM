    ...
    if (data.markdown === true) {
        ...
        if (data.type === 'danger') {
            msgHtml += `<div class="text-right">
                <small>
                    For support, visit <strong><a href="http://discord.gg/vibeSM" target="_blank" class="text-dark">discord.gg/vibeSM</a></strong>.
                </small>
            </div>`;
        }
...
const vibeSMAPI = ({type, url, data, dataType, timeout, success, error}) => {
    ...
    if (serverId) {
        url += (url.includes('?') ? '&' : '?') + 'serverId=' + encodeURIComponent(serverId);
    }
    ...
    // console.log(`vibeSMAPI Req to: ${url}`);
    return $.ajax({type, url, timeout, data, dataType, success, error, headers});
};
const txAdminAPI = vibeSMAPI;

const vibeSMAlert = ({content, modalColor, title}) => {
    $.confirm({
        title,
        content: content,
...
const vibeSMConfirm = ({content, confirmBtnClass, modalColor, title}) => {
